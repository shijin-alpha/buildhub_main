...
try {
    ...
    // Check blockchain trust records
    echo "=== Blockchain Trust Records ===\n";
    $stmt = $db->query("SELECT * FROM blockchain_trust_records ORDER BY created_at DESC");
    $blockchainRecords = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    echo "Total blockchain trust records: " . count($blockchainRecords) . "\n\n";
    
    if (count($blockchainRecords) > 0) {
        foreach ($blockchainRecords as $record) {
            echo "--- Record ID: {$record['id']} ---\n";
            echo "Payment ID: {$record['payment_id']}\n";
            echo "Operation Type: {$record['operation_type']}\n";
    ...
    // Check immutable audit ledger
    echo "=== Immutable Payment Audit Ledger ===\n";
    $stmt = $db->query("SELECT * FROM immutable_payment_audit_ledger ORDER BY block_number DESC");
    $auditRecords = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    echo "Total audit ledger entries: " . count($auditRecords) . "\n\n";
