...
try {
    ...
    // Get assigned projects (estimates that are accepted/acknowledged and ready for construction)
    $stmt = $db->prepare("
        SELECT 
    ...
    foreach ($projects as &$project) {
        ...
        $project['completed_stages'] = 0;
        $project['in_progress_stages'] = 0;
        $project['total_updates'] = 0;
        $project['daily_updates_count'] = 0;
        $project['weekly_summaries_count'] = 0;
        $project['monthly_reports_count'] = 0;
        $project['enhanced_progress'] = 0;
        $project['last_update'] = null;

        // Format dates
        $project['estimate_date_formatted'] = date('M j, Y', strtotime($project['estimate_date']));
        $project['request_date_formatted'] = $project['request_date'] ? date('M j, Y', strtotime($project['request_date'])) : 'N/A';
        ...
        $project['last_update_formatted'] = 'No updates yet';

        // Use effective status for display
        $project['project_status'] = $project['effective_status'];

        // Create project display name with details
