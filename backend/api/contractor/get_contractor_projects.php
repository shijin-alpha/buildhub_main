...
try {
    ...
    try {
        ...
        foreach ($projects as $project) {
            ...
            try {
                // Check daily updates - first try project ID, then estimate ID if no results
                ...
                // If no updates found with project_id, try estimate_id
                if ($update_counts['daily_updates_count'] === 0 && !empty($project['estimate_id']) && $project['estimate_id'] != $projectId) {
                    $daily_stmt->execute([$project['estimate_id']]);
                    $daily_result = $daily_stmt->fetch(PDO::FETCH_ASSOC);
                    $update_counts['daily_updates_count'] = (int)$daily_result['count'];
                    $update_counts['latest_update_date'] = $daily_result['latest'];
                }
                ...
                $weekly_query = "SELECT COUNT(*) as count FROM weekly_progress_summary WHERE project_id = ?";
                ...
                // If no summaries found with project_id, try estimate_id
                if ($update_counts['weekly_summaries_count'] === 0 && !empty($project['estimate_id']) && $project['estimate_id'] != $projectId) {
                    $weekly_stmt->execute([$project['estimate_id']]);
                    $weekly_result = $weekly_stmt->fetch(PDO::FETCH_ASSOC);
                    $update_counts['weekly_summaries_count'] = (int)$weekly_result['count'];
                }
                ...
                // If no reports found with project_id, try estimate_id
                if ($update_counts['monthly_reports_count'] === 0 && !empty($project['estimate_id']) && $project['estimate_id'] != $projectId) {
                    $monthly_stmt->execute([$project['estimate_id']]);
                    $monthly_result = $monthly_stmt->fetch(PDO::FETCH_ASSOC);
                    $update_counts['monthly_reports_count'] = (int)$monthly_result['count'];
                }
