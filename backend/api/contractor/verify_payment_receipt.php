...
function ensureReceiptVerificationHashTable($db) {
    $db->exec("
        CREATE TABLE IF NOT EXISTS receipt_verification_hashes (
...
function recordReceiptVerificationHash($db, $payment, $verification_status, $verification_notes, $contractor_id) {
    ensureReceiptVerificationHashTable($db);
    ...
    $insertStmt = $db->prepare("
        INSERT INTO receipt_verification_hashes (
            payment_id,
            project_id,
            contractor_id,
            verification_status,
...
try {
    ...
    // Record a lightweight hash chain entry for this verification
    $verificationHash = recordReceiptVerificationHash(
        $db,
        $payment,
        $verification_status,
        $verification_notes,
    ...
    // === BLOCKCHAIN RECEIPT VERIFICATION INTEGRATION ===
    try {
        require_once __DIR__ . '/../../blockchain/ReceiptVerificationBlockchainIntegrator.php';
        ...
        // Record receipt verification on blockchain with hashing
        $blockchainResult = $blockchainIntegrator->recordContractorReceiptVerification(
            $blockchainPaymentData, 
            $blockchainVerificationData
        );
        
        if ($blockchainResult) {
            error_log("Receipt verification successfully recorded on blockchain: Payment ID {$payment_id}, Hash: {$blockchainResult}");
        }
        
    } catch (Exception $blockchainError) {
        // Blockchain integration failure should not affect payment verification
        error_log("Blockchain receipt verification integration failed (non-critical): " . $blockchainError->getMessage());
    }
    // === END BLOCKCHAIN RECEIPT VERIFICATION INTEGRATION ===
