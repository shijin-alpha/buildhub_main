...
try {
    ...
    if ($isMultipart) {
        ...
        $incremental_completion = isset($_POST['incremental_completion_percentage']) ? round((float)$_POST['incremental_completion_percentage'], 2) : 0;
        ...
        $incremental_completion = isset($input['incremental_completion_percentage']) ? round((float)$input['incremental_completion_percentage'], 2) : 0;
    ...
    // Stage-specific validation - check if increment would exceed stage limit
    $stage_limits = [
        'Foundation' => 12.5,
        'Structure' => 12.5,
        'Brickwork' => 12.5,
    ...
    if (isset($stage_limits[$construction_stage])) {
        // Get current stage progress
        $stageProgressCheck = $db->prepare("
            SELECT COALESCE(SUM(incremental_completion_percentage), 0) as current_stage_progress
            FROM daily_progress_updates 
            WHERE project_id = :project_id AND contractor_id = :contractor_id AND construction_stage = :stage
        ");
        $stageProgressCheck->bindValue(':project_id', $project_id, PDO::PARAM_INT);
        $stageProgressCheck->bindValue(':contractor_id', $contractor_id, PDO::PARAM_INT);
        $stageProgressCheck->bindValue(':stage', $construction_stage, PDO::PARAM_STR);
        $stageProgressCheck->execute();
        $currentStageProgress = $stageProgressCheck->fetchColumn();
        
        $stage_limit = $stage_limits[$construction_stage];
        $remaining_in_stage = $stage_limit - $currentStageProgress;
        
        // Allow small tolerance for rounding errors (0.1%)
        if ($incremental_completion > $remaining_in_stage + 0.1) {
            // Auto-cap to remaining amount instead of rejecting
            $original_increment = $incremental_completion;
            $incremental_completion = round($remaining_in_stage, 2);
            
            // If remaining is too small (less than 0.01%), complete the stage
            if ($incremental_completion < 0.01) {
                $incremental_completion = $remaining_in_stage;
            }
            
            error_log("Stage limit auto-cap: {$construction_stage} stage - Original: {$original_increment}%, Capped: {$incremental_completion}%, Remaining was: {$remaining_in_stage}%");
        }
    }
    ...
    $cumulative_completion = round(min(100, $previous_cumulative + $incremental_completion), 2);
    ...
    try {
        ...
        // Update construction_projects with latest progress
        $updateProjectStmt = $db->prepare("
            UPDATE construction_projects 
            SET completion_percentage = :completion,
                current_stage = :stage,
                status = CASE WHEN :completion2 >= 100 THEN 'completed' ELSE 'in_progress' END,
                last_update_date = NOW(),
                updated_at = NOW()
            WHERE id = :project_id
        ");
        $updateProjectStmt->bindValue(':completion', $cumulative_completion, PDO::PARAM_STR);
        $updateProjectStmt->bindValue(':completion2', $cumulative_completion, PDO::PARAM_STR);
        $updateProjectStmt->bindValue(':stage', $construction_stage, PDO::PARAM_STR);
        $updateProjectStmt->bindValue(':project_id', $project_id, PDO::PARAM_INT);
