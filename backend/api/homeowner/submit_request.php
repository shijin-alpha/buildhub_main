...
try {
    ...
    // Kerala-specific ML fields
    $kerala_district = $input['kerala_district'] ?? null;
    $construction_start_month = isset($input['construction_start_month']) ? intval($input['construction_start_month']) : null;
    // Clamp month to valid range
    if ($construction_start_month !== null && ($construction_start_month < 1 || $construction_start_month > 12)) {
        $construction_start_month = null;
    }

    // AI prediction fields (from final review step)
    $ai_prediction_data = isset($input['ai_prediction']) && is_array($input['ai_prediction']) ? $input['ai_prediction'] : null;
    $ai_cost_risk_level  = $ai_prediction_data['final_risk'] ?? ($ai_prediction_data['cost_overrun_risk']['risk_level'] ?? null);
    $ai_time_risk_level  = $ai_prediction_data['time_delay_risk']['risk_level'] ?? null;
    $ai_risk_score       = isset($ai_prediction_data['risk_score']) ? floatval($ai_prediction_data['risk_score']) : null;
    $ai_prediction_json  = $ai_prediction_data ? json_encode($ai_prediction_data, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : null;
    ...
    $db->exec("ALTER TABLE layout_requests ADD COLUMN IF NOT EXISTS kerala_district VARCHAR(50) DEFAULT NULL");
    $db->exec("ALTER TABLE layout_requests ADD COLUMN IF NOT EXISTS construction_start_month TINYINT UNSIGNED DEFAULT NULL");
    ...
    if (!$requestId) {
        ...
        $query = "INSERT INTO layout_requests (
                    ...
                    ai_cost_risk_level, ai_time_risk_level, ai_risk_score, ai_prediction_data
                  ) VALUES (
                    :user_id, :homeowner_id, :plot_size, :building_size, :budget_range, :requirements, :location, :timeline, :selected_layout_id, :layout_type,
                    :orientation, :site_considerations, :material_preferences, :budget_allocation, :num_floors, :preferred_style, :floor_rooms,
                    :site_images, :reference_images, :room_images,
        ...
        $stmt->bindParam(':kerala_district', $kerala_district);
        $stmt->bindParam(':construction_start_month', $construction_start_month, PDO::PARAM_INT);
        $stmt->bindParam(':ai_cost_risk_level', $ai_cost_risk_level);
        $stmt->bindParam(':ai_time_risk_level', $ai_time_risk_level);
        $stmt->bindParam(':ai_risk_score', $ai_risk_score);
        $stmt->bindParam(':ai_prediction_data', $ai_prediction_json);
