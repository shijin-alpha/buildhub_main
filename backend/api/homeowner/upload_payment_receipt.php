...
try {
    ...
    // UNIVERSAL FIX: Auto-establish session for any homeowner based on payment ownership
    if (!$homeowner_id) {
        ...
        if ($payment_id) {
            ...
            if ($tempResult) {
                ...
                error_log("UNIVERSAL FIX: Auto-logged in homeowner {$homeowner_id} ({$tempResult['first_name']} {$tempResult['last_name']}) for payment {$payment_id}");
            }
        }
    }
    ...
    error_log("Session user_id: " . ($homeowner_id ?? 'NULL'));
    ...
    if (!$payment) {
        ...
        $customDebugStmt = $db->prepare("
            SELECT id, homeowner_id, contractor_id, requested_amount, status 
            FROM custom_payment_requests 
            WHERE id = :payment_id
        ");
        $customDebugStmt->execute([':payment_id' => $payment_id]);
        $customDebugPayment = $customDebugStmt->fetch(PDO::FETCH_ASSOC);
        
        error_log("Payment lookup failed - ID: $payment_id, Homeowner: $homeowner_id");
        if ($debugPayment) {
            error_log("Stage payment exists but belongs to homeowner: " . $debugPayment['homeowner_id']);
        }
        if ($customDebugPayment) {
            error_log("Custom payment exists but belongs to homeowner: " . $customDebugPayment['homeowner_id']);
        }
        
        // Provide specific error messages
        $errorMessage = 'Payment not found or access denied';
        $debugInfo = [
            'payment_id' => $payment_id,
            'homeowner_id' => $homeowner_id,
            'stage_payment_exists' => $debugPayment ? true : false,
            'custom_payment_exists' => $customDebugPayment ? true : false,
            'stage_payment_details' => $debugPayment,
            'custom_payment_details' => $customDebugPayment
        ];
        
        if ($debugPayment && $debugPayment['homeowner_id'] != $homeowner_id) {
            $errorMessage = 'This payment belongs to a different homeowner. Please refresh the page and try again.';
        } elseif ($customDebugPayment && $customDebugPayment['homeowner_id'] != $homeowner_id) {
            $errorMessage = 'This payment belongs to a different homeowner. Please refresh the page and try again.';
        } elseif (!$debugPayment && !$customDebugPayment) {
            $errorMessage = 'Payment ID ' . $payment_id . ' does not exist. Please refresh the page and select a valid payment.';
        }
        ...
        echo json_encode([
            ...
            'debug' => $debugInfo,
            'suggestion' => 'Please refresh the page to load the latest payment requests and try again.'
        ]);
        exit;
    }
