...
try {
    ...
    // Get project information - fetch even if no timeline data yet
    $project_info = null;
    if (!empty($timeline_data)) {
    ...
    // If no timeline data, still fetch all construction projects for this homeowner
    $all_projects = [];
    if (empty($timeline_data)) {
        $all_proj_stmt = $db->prepare("
            SELECT 
                id,
                project_name,
                project_description as project_type,
                ...
                completion_percentage,
                current_stage,
                total_cost as estimate_cost
            FROM construction_projects 
            WHERE homeowner_id = ?
            AND status IN ('created', 'in_progress')
            ORDER BY id DESC
        ");
        $all_proj_stmt->execute([$homeowner_id]);
        $all_projects = $all_proj_stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Also check contractor_estimates accepted for this homeowner
        if (empty($all_projects)) {
            $est_stmt = $db->prepare("
                SELECT 
                    ce.id,
                    ce.project_name,
                    ce.notes as project_type,
                ...
                WHERE ce.homeowner_id = ? AND ce.status IN ('accepted', 'construction_started')
                ORDER BY ce.id DESC
            ");
            $est_stmt->execute([$homeowner_id]);
            $all_projects = $est_stmt->fetchAll(PDO::FETCH_ASSOC);
        }
    }
