...
try {
    ...
    $checkStmt = $db->prepare("
        SELECT cse.id, cse.contractor_id, cse.status, cls.homeowner_id,
               cse.total_cost, cse.timeline, cse.notes,
               u.first_name, u.last_name, u.email,
               hu.first_name as hw_first, hu.last_name as hw_last, hu.email as hw_email, hu.phone as hw_phone,
               lr.budget_range, lr.plot_size, lr.location, lr.preferred_style,
               CASE 
                   WHEN cse.structured IS NOT NULL AND JSON_UNQUOTE(JSON_EXTRACT(cse.structured, '$.project_name')) != 'null'
                   THEN JSON_UNQUOTE(JSON_EXTRACT(cse.structured, '$.project_name'))
                   ELSE CONCAT(hu.first_name, ' ', hu.last_name, ' Construction Project')
               END as project_name
        FROM contractor_send_estimates cse
        INNER JOIN contractor_layout_sends cls ON cse.send_id = cls.id
        LEFT JOIN users u ON cse.contractor_id = u.id
        LEFT JOIN users hu ON cls.homeowner_id = hu.id
    ...
    if ($updateStmt->execute()) {
        ...
        // Check if a construction_projects row already exists for this estimate
        $existsStmt = $db->prepare("SELECT id FROM construction_projects WHERE estimate_id = ? LIMIT 1");
        $existsStmt->execute([$estimate_id]);
        $existing = $existsStmt->fetch(PDO::FETCH_ASSOC);

        if (!$existing) {
            // Calculate expected completion date from timeline string (e.g. "6 months")
            $expected_completion = null;
            if (!empty($estimate['timeline'])) {
                preg_match('/(\d+)\s*(month|week|day)/i', $estimate['timeline'], $m);
                if (!empty($m)) {
                    $num = (int)$m[1];
                    $unit = strtolower($m[2]);
                    $interval = $unit === 'month' ? "P{$num}M" : ($unit === 'week' ? "P{$num}W" : "P{$num}D");
                    try {
                        $expected_completion = (new DateTime())->add(new DateInterval($interval))->format('Y-m-d');
                    } catch (Exception $e) {}
                }
            }
            ...
            $projStmt = $db->prepare("
                INSERT INTO construction_projects (
                    ...
                    total_cost, timeline, contractor_notes,
                    homeowner_name, homeowner_email, homeowner_phone, project_location,
                    plot_size, budget_range, preferred_style,
                    status, current_stage, completion_percentage,
                    start_date, expected_completion_date
                ) VALUES (
                    :estimate_id, :contractor_id, :homeowner_id, :project_name, :project_description,
                    :total_cost, :timeline, :notes,
                    :homeowner_name, :homeowner_email, :homeowner_phone, :project_location,
                    :plot_size, :budget_range, :preferred_style,
                    'created', 'Planning', 0,
                    CURDATE(), :expected_completion
                )
            ");
            $projStmt->execute([
                ...
                ':project_description' => $estimate['notes'],
                ':total_cost'        => $estimate['total_cost'],
                ':timeline'          => $estimate['timeline'],
                ':notes'             => $estimate['notes'],
                ':homeowner_name'    => trim($estimate['hw_first'] . ' ' . $estimate['hw_last']),
                ':homeowner_email'   => $estimate['hw_email'],
                ':homeowner_phone'   => $estimate['hw_phone'],
                ':project_location'  => $estimate['location'],
                ':plot_size'         => $estimate['plot_size'],
                ':budget_range'      => $estimate['budget_range'],
                ':preferred_style'   => $estimate['preferred_style'],
            ...
            $db->prepare("UPDATE construction_projects SET status = 'created', start_date = COALESCE(start_date, CURDATE()) WHERE id = ?")->execute([$project_id]);
        }
