...
try {
    ...
    if ($homeowner_id) {
        try {
            ...
            if ($layoutRequestRow) {
                ...
                // Parse ai_prediction_data if stored as JSON string
                if (!empty($layoutRequestRow['ai_prediction_data'])) {
                    try {
                        $layout_request_details['ai_prediction_data'] = json_decode($layoutRequestRow['ai_prediction_data'], true);
                    } catch (Throwable $e) {}
                }
            }
        } catch (Throwable $e) {
            error_log("Failed to get layout request details: " . $e->getMessage());
            // Try without AI prediction columns (older DB)
            try {
                $layoutRequestStmt = $db->prepare("
                    SELECT 
                        plot_size, building_size, budget_range, location, timeline, 
                ...
                error_log("Failed to get layout request details (fallback): " . $e2->getMessage());
            }
        }
    }
    ...
    $payload = [
        ...
        // AI predictions — included for contractor, excluded from architect flow
        'ai_predictions' => $layout_request_details ? [
            'cost_risk_level'  => $layout_request_details['ai_cost_risk_level'] ?? null,
            'time_risk_level'  => $layout_request_details['ai_time_risk_level'] ?? null,
            'risk_score'       => $layout_request_details['ai_risk_score'] ?? null,
            'prediction_data'  => $layout_request_details['ai_prediction_data'] ?? null,
        ] : null,
    ];
