...
try {
    ...
    // Get homeowner ID from session or parameter
    $homeowner_id = 0;
    if (isset($_SESSION['user_id']) && $_SESSION['role'] === 'homeowner') {
        $homeowner_id = (int)$_SESSION['user_id'];
    } else if (isset($_GET['homeowner_id'])) {
    ...
    if ($homeowner_id <= 0) {
        echo json_encode(['success' => false, 'message' => 'Missing homeowner_id or invalid session']);
        exit;
    }
