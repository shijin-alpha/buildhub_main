...
try {
    ...
    error_log("Payment initiation - Request ID: $payment_request_id, Amount received: " . var_export($amount, true) . " (type: " . gettype($amount) . ")");
    ...
    if (!$request) {
        echo json_encode(['success' => false, 'message' => 'Payment request not found or not eligible for payment']);
        exit;
    }
    ...
    try {
        ...
        if ($cancelAltStmt->rowCount() > 0) {
            error_log("Cancelled " . $cancelAltStmt->rowCount() . " pending alternative payments for request ID: $payment_request_id");
        }
    } catch (Exception $e) {
    ...
    if ($input_amount > $max_allowed_amount) {
        echo json_encode([
            'success' => false, 
            'message' => "Payment amount ₹" . number_format($input_amount, 2) . " exceeds maximum allowed amount of ₹" . number_format($max_allowed_amount, 2) . " for this stage"
        ]);
        exit;
    }
    ...
    // Create stage payment payments table if it doesn't exist
    $db->exec("
    ...
    error_log("Initiate stage payment error: " . $e->getMessage());
    echo json_encode([
        'success' => false,
