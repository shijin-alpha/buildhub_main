<?php
/**
 * Complete ALL Construction Stages for Project 37
...
try {
    ...
    echo "�  Contractor: {$project['contractor_id']}\n";
    echo "🏠 Homeowner: {$project['homeowner_id']}\n\n";
    ...
    foreach ($stages as $stage) {
        ...
        for ($day = 1; $day <= 5; $day++) {
            ...
            if (!$existing) {
                ...
                if ($day == 1) {
                    $workDesc .= "Started work, site preparation and material delivery completed.";
                } elseif ($day == 2) {
                    $workDesc .= "Main construction work in progress, all materials on site.";
                } elseif ($day == 3) {
                    $workDesc .= "Halfway through stage, quality checks performed and passed.";
                } elseif ($day == 4) {
                    $workDesc .= "Nearing completion, final touches and cleanup in progress.";
                } elseif ($day == 5) {
                    $workDesc .= "Stage completed successfully, ready for inspection.";
                }
                ...
                ) VALUES (?, ?, ?, ?, ?, ?, 10, 'Clear and sunny', 'No major issues', '[]', NOW())");
                
                $stmt->execute([$projectId, $contractorId, $stage['name'], $reportDate, $progress, $workDesc]);
                $reportsCreated++;
                $totalReports++;
            }
        }
    ...
    foreach ($stages as $stage) {
        ...
        if (!$existing) {
            ...
            $stmt->execute([
                ...
                $stage['name'], $stage['order'], $stage['cost'], $description
            ]);
    ...
    foreach ($stages as $stage) {
        ...
        if (!$existing) {
            ...
            ) VALUES (?, ?, ?, NOW(), 'approved', 5, 5, 5, ?, 
                'Continue to next stage. Maintain current quality standards and safety protocols.', NOW())");
            
            $stmt->execute([$projectId, $inspectorId, $stage['name'], $notes]);
    ...
    foreach ($stages as $stage) {
        $documents = [
            ...
            [
                ...
                'name' => "Completion Certificate - {$stage['name']}",
                'desc' => "Official completion certificate for {$stage['name']} stage"
            ]
        ];
        ...
        foreach ($documents as $doc) {
            ...
            if (!$existing) {
                ...
                $stmt->execute([
                    ...
                    $doc['type'], $doc['name'], $doc['desc'], $filePath
                ]);
