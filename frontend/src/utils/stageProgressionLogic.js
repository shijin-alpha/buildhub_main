...
export const getAvailableStages = (stageBreakdown) => {
  ...
  CONSTRUCTION_STAGES.forEach(stage => {
    ...
    // Only include stages that are NOT completed
    if (!stageData.is_completed) {
      // Check if this stage can be worked on
      const stageIndex = CONSTRUCTION_STAGES.findIndex(s => s.name === stage.name);
      
      // Allow current stage and next stage if current is nearly complete
      if (stageIndex === 0) {
        // First stage (Foundation) is always available if not completed
        availableStages.push({
          ...stage,
          is_current: stageData.is_active,
          remaining_percentage: Math.max(0, stage.percentage - stageData.current_progress),
          current_progress: stageData.current_progress,
          is_completed: stageData.is_completed
        });
      } else {
        // Check if previous stage is completed or nearly completed
        const previousStage = CONSTRUCTION_STAGES[stageIndex - 1];
        const previousStageData = stages[previousStage.name];
        
        if (previousStageData.is_completed || previousStageData.current_progress >= previousStage.percentage * 0.8) {
          availableStages.push({
            ...stage,
            is_current: stageData.is_active,
            remaining_percentage: Math.max(0, stage.percentage - stageData.current_progress),
...
export const validateDailyProgress = (selectedStage, incrementalPercentage, stageBreakdown) => {
  ...
  const currentProgress = stageData.current_progress || 0;
  const newTotal = currentProgress + incrementalPercentage;
  const remaining = stage.percentage - currentProgress;
  
  // Add small tolerance for rounding errors (0.1%)
  const tolerance = 0.1;
  const effectiveLimit = stage.percentage + tolerance;
  
  // Check if exceeds stage limit (with tolerance)
  if (newTotal > effectiveLimit) {
    if (remaining <= 0) {
      errors.push(`${selectedStage} stage is already completed (${currentProgress.toFixed(1)}% of ${stage.percentage}%)`);
    } else {
      errors.push(`This stage can only accept ${remaining.toFixed(1)}% more progress. Current: ${currentProgress.toFixed(1)}%, Limit: ${stage.percentage}%`);
    }
    return { isValid: false, errors, warnings };
  }
  ...
  // Warn if approaching stage limit
  if (newTotal > stage.percentage * 0.9 && newTotal < stage.percentage) {
    warnings.push(`This stage is almost complete. Only ${remaining.toFixed(1)}% remaining.`);
  }
  ...
  return {
    ...
    remaining_in_stage: Math.max(0, stage.percentage - newTotal),
    current_stage_progress: currentProgress,
    max_allowed_increment: remaining
  };
};
