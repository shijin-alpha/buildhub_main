...
const ContractorConstructionTimeline = ({ contractorId, projectId = null, homeownerId = null }) => {
  ...
  const getUniqueProjects = () => {
    ...
    // Seed from projectsInfo (authoritative source with real progress from DB)
    Object.entries(projectsInfo).forEach(([id, info]) => {
      projects[id] = {
        id: parseInt(id),
        name: info.project_name || `Project ${id}`,
        homeowner_id: info.homeowner_id,
        latest_progress: parseFloat(info.completion_percentage) || 0,
        updates_count: 0
      };
    });

    // Overlay updates_count and latest_date from statistics.projects_summary
    const summary = statistics.projects_summary || [];
    summary.forEach(s => {
      const key = String(s.project_id);
      if (projects[key]) {
        projects[key].updates_count = s.updates_count || 0;
        if (s.latest_progress > projects[key].latest_progress) {
          projects[key].latest_progress = s.latest_progress;
        }
      }
    });
  ...
  // Check if we have projects even without timeline updates
  const uniqueProjects = getUniqueProjects();

  if (timelineData.length === 0 && uniqueProjects.length === 0) {
    return (
  ...
  return (
    <div className="construction-timeline contractor-timeline">
          ...
          >
            ...
            {uniqueProjects.map(project => (
              <option key={project.id} value={project.id}>
                📋 {getProjectDisplayName(project)} • {project.updates_count > 0 ? `${project.updates_count} updates • ${project.latest_progress}% complete` : 'New — no updates yet'}
              </option>
            ))}
          </select>
        </div>
      ...
      {/* Projects Overview */}
      {!selectedProject && uniqueProjects.length > 0 && (
            ...
            {uniqueProjects.map(project => (
              <div key={project.id} className="project-card" onClick={() => setSelectedProject(project.id)}>
                ...
                <div className="project-details">
                  <p>{project.updates_count > 0 ? `${project.updates_count} progress updates` : 'No updates yet — ready to start'}</p>
                  <div className="mini-progress-bar">
                    <div 
                      className="mini-progress-fill"
                      style={{ 
                        width: `${project.latest_progress}%`,
                        backgroundColor: project.updates_count === 0 ? '#9ca3af' : getProgressColor(project.latest_progress)
                      }}
                    ></div>
                  </div>
                </div>
