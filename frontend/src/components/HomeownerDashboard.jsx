...
import '../styles/ImageErrorHandling.css';
...
const HomeownerDashboard = () => {
  ...
  // Add CSS for loading spinner and image error handling
  useEffect(() => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const response = await fetch('/buildhub/backend/api/homeowner/get_dashboard_data.php', {
        method: 'GET',
        credentials: 'include',
        headers: {
      ...
      if (result.success) {
        ...
        renderProgressOverview(result.data);
        renderBudgetTracker(result.data);
      } else {
        setDashboardError(result.message);
        showError('progress-overview', result.message);
        showError('budget-tracker', result.message);
      }
    } catch (error) {
      console.error('Error loading dashboard data:', error);
      setDashboardError('Failed to load dashboard data');
      showError('progress-overview', 'Failed to load dashboard data');
      showError('budget-tracker', 'Failed to load dashboard data');
    } finally {
      setDashboardLoading(false);
    }
  };

  // Load dashboard data on component mount and when activeTab changes to dashboard
  useEffect(() => {
  ...
  // Helper function to show error in widget
  const showError = (containerId, message) => {
    const container = document.getElementById(containerId);
    if (container) {
      container.innerHTML = `<div class="error-message" style="background: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; margin: 20px 0; border: 1px solid #f5c6cb;">Error: ${message}</div>`;
    }
  };
  ...
  const renderProgressOverview = (data) => {
    ...
    if (!data.projects || data.projects.length === 0) {
      container.innerHTML = '<div class="empty-state" style="text-align: center; padding: 60px 20px; color: #6c757d; font-size: 1.1rem;"><span style="font-size: 3rem; display: block; margin-bottom: 15px;">📊</span>No active projects found</div>';
      return;
    }
    ...
    data.projects.forEach(project => {
      const progressPercentage = project.progress.completion_percentage || 0;
      const daysRemaining = project.timeline.days_remaining;
      const statusClass = project.progress.status === 'in_progress' ? 'status-in-progress' : 'status-created';
      
      html += `
        <div class="project-card" style="background: linear-gradient(135deg, #fff 0%, #f8f9fa 100%); border: 1px solid #e9ecef; border-radius: 12px; padding: 25px; margin-bottom: 20px; transition: all 0.3s ease;">
          <div class="project-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
            <div>
              <div class="project-title" style="font-size: 1.4rem; font-weight: 600; color: #2c3e50; margin-bottom: 5px;">${project.name}</div>
              <div class="project-location" style="color: #6c757d; font-size: 0.95rem;">${project.location || 'Location not specified'}</div>
            </div>
            <div class="project-status ${statusClass}" style="padding: 6px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; text-transform: uppercase; ${statusClass === 'status-in-progress' ? 'background: #d4edda; color: #155724;' : 'background: #d1ecf1; color: #0c5460;'}">
              ${project.progress.status.replace('_', ' ')}
            </div>
          </div>
          ...
          <div class="progress-section" style="margin-bottom: 20px;">
            <div class="progress-info" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
              <span class="current-stage" style="font-weight: 600; color: #495057;">Current Stage: ${project.progress.current_stage}</span>
              <span class="progress-percentage" style="font-weight: 700; color: #28a745; font-size: 1.1rem;">${progressPercentage}%</span>
            </div>
            <div class="progress-bar" style="width: 100%; height: 8px; background: #e9ecef; border-radius: 4px; overflow: hidden;">
              <div class="progress-fill" style="height: 100%; background: linear-gradient(90deg, #28a745 0%, #20c997 100%); border-radius: 4px; transition: width 0.8s ease; width: ${progressPercentage}%;"></div>
            </div>
          </div>
          
          <div class="project-details" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 20px;">
            <div class="detail-item" style="text-align: center; padding: 15px; background: rgba(108, 117, 125, 0.1); border-radius: 8px;">
              <div class="detail-label" style="font-size: 0.85rem; color: #6c757d; margin-bottom: 5px; text-transform: uppercase; font-weight: 600;">Contractor</div>
              <div class="detail-value" style="font-size: 1.1rem; font-weight: 600; color: #2c3e50;">${project.contractor.name || 'Not assigned'}</div>
            </div>
            <div class="detail-item" style="text-align: center; padding: 15px; background: rgba(108, 117, 125, 0.1); border-radius: 8px;">
              <div class="detail-label" style="font-size: 0.85rem; color: #6c757d; margin-bottom: 5px; text-transform: uppercase; font-weight: 600;">Timeline</div>
              <div class="detail-value" style="font-size: 1.1rem; font-weight: 600; color: #2c3e50;">${project.timeline.timeline_text || 'Not specified'}</div>
            </div>
            <div class="detail-item" style="text-align: center; padding: 15px; background: rgba(108, 117, 125, 0.1); border-radius: 8px;">
              <div class="detail-label" style="font-size: 0.85rem; color: #6c757d; margin-bottom: 5px; text-transform: uppercase; font-weight: 600;">Days Remaining</div>
              <div class="detail-value" style="font-size: 1.1rem; font-weight: 600; color: #2c3e50;">${daysRemaining !== null ? daysRemaining + ' days' : 'N/A'}</div>
            </div>
            <div class="detail-item" style="text-align: center; padding: 15px; background: rgba(108, 117, 125, 0.1); border-radius: 8px;">
              <div class="detail-label" style="font-size: 0.85rem; color: #6c757d; margin-bottom: 5px; text-transform: uppercase; font-weight: 600;">Total Cost</div>
              <div class="detail-value" style="font-size: 1.1rem; font-weight: 600; color: #2c3e50;">₹${formatCurrency(project.budget.total_cost)}</div>
            </div>
          </div>
        </div>
      `;
  ...
  // Render budget tracker widget
  const renderBudgetTracker = (data) => {
    const container = document.getElementById('budget-tracker');
    if (!container) return;
    
    if (!data.budget_summary) {
      container.innerHTML = '<div class="empty-state" style="text-align: center; padding: 60px 20px; color: #6c757d; font-size: 1.1rem;"><span style="font-size: 3rem; display: block; margin-bottom: 15px;">📊</span>No budget data available</div>';
      return;
    }

    const budget = data.budget_summary;
    const utilizationPercentage = budget.utilization_percentage || 0;
    const pendingPercentage = budget.pending_percentage || 0;
    const remainingPercentage = 100 - utilizationPercentage - pendingPercentage;

    let html = `
      <div class="budget-overview" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 20px; margin-bottom: 25px;">
        <div class="budget-item" style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border: 1px solid #dee2e6;">
          <div class="budget-amount paid" style="font-size: 1.5rem; font-weight: 700; margin-bottom: 5px; color: #28a745;">₹${formatCurrency(budget.total_paid)}</div>
          <div class="budget-label" style="font-size: 0.9rem; color: #6c757d; text-transform: uppercase; font-weight: 600;">Paid</div>
        </div>
        <div class="budget-item" style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border: 1px solid #dee2e6;">
          <div class="budget-amount pending" style="font-size: 1.5rem; font-weight: 700; margin-bottom: 5px; color: #ffc107;">₹${formatCurrency(budget.total_pending)}</div>
          <div class="budget-label" style="font-size: 0.9rem; color: #6c757d; text-transform: uppercase; font-weight: 600;">Pending</div>
        </div>
        <div class="budget-item" style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border: 1px solid #dee2e6;">
          <div class="budget-amount remaining" style="font-size: 1.5rem; font-weight: 700; margin-bottom: 5px; color: #6c757d;">₹${formatCurrency(budget.total_remaining)}</div>
          <div class="budget-label" style="font-size: 0.9rem; color: #6c757d; text-transform: uppercase; font-weight: 600;">Remaining</div>
        </div>
        <div class="budget-item" style="text-align: center; padding: 20px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border: 1px solid #dee2e6;">
          <div class="budget-amount" style="font-size: 1.5rem; font-weight: 700; margin-bottom: 5px; color: #2c3e50;">₹${formatCurrency(budget.total_budget)}</div>
          <div class="budget-label" style="font-size: 0.9rem; color: #6c757d; text-transform: uppercase; font-weight: 600;">Total Budget</div>
        </div>
      </div>

      <div class="budget-chart" style="margin: 25px 0; padding: 20px; background: #f8f9fa; border-radius: 10px;">
        <div class="chart-bar" style="width: 100%; height: 20px; background: #e9ecef; border-radius: 10px; overflow: hidden; display: flex;">
          <div class="chart-segment chart-paid" style="height: 100%; background: linear-gradient(90deg, #28a745 0%, #20c997 100%); transition: width 0.8s ease; width: ${utilizationPercentage}%;"></div>
          <div class="chart-segment chart-pending" style="height: 100%; background: linear-gradient(90deg, #ffc107 0%, #fd7e14 100%); transition: width 0.8s ease; width: ${pendingPercentage}%;"></div>
          <div class="chart-segment chart-remaining" style="height: 100%; background: linear-gradient(90deg, #6c757d 0%, #495057 100%); transition: width 0.8s ease; width: ${remainingPercentage}%;"></div>
        </div>
        <div class="chart-legend" style="display: flex; justify-content: center; gap: 20px; margin-top: 15px;">
          <div class="legend-item" style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem;">
            <div class="legend-color" style="width: 12px; height: 12px; border-radius: 2px; background: linear-gradient(90deg, #28a745 0%, #20c997 100%);"></div>
            <span>Paid (${utilizationPercentage.toFixed(1)}%)</span>
          </div>
          <div class="legend-item" style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem;">
            <div class="legend-color" style="width: 12px; height: 12px; border-radius: 2px; background: linear-gradient(90deg, #ffc107 0%, #fd7e14 100%);"></div>
            <span>Pending (${pendingPercentage.toFixed(1)}%)</span>
          </div>
          <div class="legend-item" style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem;">
            <div class="legend-color" style="width: 12px; height: 12px; border-radius: 2px; background: linear-gradient(90deg, #6c757d 0%, #495057 100%);"></div>
    ...
    if (data.recent_activity && data.recent_activity.payments.length > 0) {
      html += `
        <div class="recent-activity" style="margin-top: 25px;">
          <h4 style="margin-bottom: 15px; color: #2c3e50;">Recent Payment Activity</h4>
      `;
      
      data.recent_activity.payments.forEach(payment => {
        const statusColor = payment.status === 'paid' ? '#28a745' : 
                           payment.status === 'pending' ? '#ffc107' : '#6c757d';
        
        html += `
          <div class="activity-item" style="display: flex; justify-content: space-between; align-items: center; padding: 15px; border-bottom: 1px solid #e9ecef; transition: background 0.3s ease;">
            <div class="activity-info" style="flex: 1;">
              <div class="activity-title" style="font-weight: 600; color: #2c3e50; margin-bottom: 3px;">${payment.stage_name} - ${payment.project_name}</div>
              <div class="activity-subtitle" style="font-size: 0.9rem; color: #6c757d;">Contractor: ${payment.contractor_name}</div>
            </div>
            <div style="text-align: right;">
              <div class="activity-amount" style="font-weight: 700; color: #28a745;">₹${formatCurrency(payment.amount)}</div>
              <div class="activity-date" style="font-size: 0.85rem; color: ${statusColor};">
                ${payment.status.toUpperCase()} • ${payment.days_ago} days ago
              </div>
            </div>
          </div>
        `;
  ...
  // Show budget details function
  const showBudgetDetails = () => {
    if (dashboardData) {
      alert('Budget details:\n\n' + 
            `Total Projects: ${dashboardData.overview.total_projects}\n` +
            `Total Budget: ₹${formatCurrency(dashboardData.budget_summary.total_budget)}\n` +
            `Amount Paid: ₹${formatCurrency(dashboardData.budget_summary.total_paid)}\n` +
            `Pending Payments: ₹${formatCurrency(dashboardData.budget_summary.total_pending)}\n` +
            `Budget Utilization: ${dashboardData.budget_summary.utilization_percentage}%`);
    }
  };
  ...
  const respondToEstimate = async (est, action, message = '') => {
    try {
      ...
      // INSTANT CONFIRMATION - Show success message immediately
      ...
      // Optimistically update the estimate status in UI
      setHomeownerEstimates(prevEstimates => 
        prevEstimates.map(estimate => 
          estimate.id === est.id 
            ? { ...estimate, status: action === 'accept' ? 'accepted' : action === 'reject' ? 'rejected' : estimate.status }
            : estimate
        )
      );
      ...
      if (!j?.success) {
        ...
        setHomeownerEstimates(prevEstimates => 
          prevEstimates.map(estimate => 
            estimate.id === est.id 
              ? { ...estimate, status: est.status } // Revert to original status
              : estimate
          )
        );
      }
      
      // Refresh list in background to sync with server
      try {
        const r = await fetch(`/buildhub/backend/api/homeowner/get_estimates.php?homeowner_id=${me.id}`, { credentials: 'include' });
      ...
      console.error('Error responding to estimate:', e);
      try { toast.error('Network error. Please try again.'); } catch { /* no-op */ }
    }
  };
  ...
  const startConstruction = async (est) => {
    try {
      ...
      // Optimistically update the estimate status
      setHomeownerEstimates(prevEstimates => 
        prevEstimates.map(estimate => 
          estimate.id === est.id 
            ? { ...estimate, construction_started: 1 }
            : estimate
        )
      );
      ...
      if (!j?.success) {
        ...
        setHomeownerEstimates(prevEstimates => 
          prevEstimates.map(estimate => 
            estimate.id === est.id 
              ? { ...estimate, construction_started: est.construction_started }
              : estimate
          )
        );
      }
      
      // Refresh estimates in background to sync with server
      try {
        const r = await fetch(`/buildhub/backend/api/homeowner/get_estimates.php?homeowner_id=${me.id}`, { credentials: 'include' });
        const k = await r.json().catch(() => ({}));
        if (k?.success) setHomeownerEstimates(Array.isArray(k.estimates) ? k.estimates : []);
          ...
          <div className="widgets-grid">
            <div className="widget-container">
              <div className="widget-header">
                ...
                <div className="widget-actions">
                  <button className="icon-btn" title="Refresh" onClick={loadDashboardData}>↻</button>
                </div>
              </div>
              <div className="widget-content" id="progress-overview">
            ...
            <div className="widget-container">
              <div className="widget-header">
                ...
                <div className="widget-actions">
                  <button className="icon-btn" title="View Details" onClick={showBudgetDetails}>👁️</button>
                </div>
              </div>
              <div className="widget-content" id="budget-tracker">
                <div className="loading-spinner" style={{ display: 'none' }}></div>
              ...
              {conceptPreviews.map(preview => (
                <div key={preview.id} className="concept-preview-card" style={{
                  ...
                  <div style={{ 
                    ...
                    <div style={{ 
                      display: preview.status === 'completed' && preview.image_url ? 'none' : 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      textAlign: 'center',
                      color: '#666',
                      position: preview.status === 'completed' && preview.image_url ? 'absolute' : 'static',
                      top: 0,
                      left: 0,
                      right: 0,
                      bottom: 0,
                                ...
                                <div key={idx} className="file-card" style={{ cursor: 'default' }}>
                                  {isImage ? (
                                    <img 
                                      ...
                                      onError={(e) => handleImageError(e, 'Layout image not available')}
                                      onLoad={handleImageLoad}
                                    />
                                  ) : null}
                                  {isImage && (
                                ...
                                <div key={idx} className="file-card" style={{ cursor: 'default' }}>
                                  {isImage ? (
                                    <img 
                                      ...
                                      style={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 6 }} 
                                      onClick={() => setViewer({ open: true, src: href, title: f.original || f.stored })} 
                                      onError={(e) => handleImageError(e, 'Design image not available')}
                                      onLoad={handleImageLoad}
                                    />
                                  ) : null}
                                  {isImage && (
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className={`dashboard-main soft-main shifted`}>
        ...
        {/* Request Form Modal */}
        {showRequestForm && (
          <div className="form-modal" style={{ backgroundColor: 'rgba(15,23,42,0.7)', backdropFilter: 'blur(4px)' }}>
            <div className="form-content" style={{ maxWidth: '860px', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', border: 'none', padding: 0 }}>
              {/* Professional Header */}
              <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)', padding: '28px 32px', position: 'relative' }}>
                <button
                  type="button"
                  onClick={() => { setShowRequestForm(false); setSelectedLibraryLayout(null); setRequestData({ plot_size: '', building_size: '', budget_range: '', plot_shape: '', num_floors: '', topography: '', development_laws: '', family_needs: [], rooms: [], aesthetic: '', style_preferences: {}, location: '', district: '', panchayat_municipality: '', timeline: '', requirements: '', selected_layout_id: null, layout_type: 'custom', custom_budget: '' }); }}
                  style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '18px', color: 'white', transition: 'background 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.25)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.15)'}
                >×</button>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' }}>
                    {requestData.layout_type === 'library' ? '📚' : '🏗️'}
                  </div>
                  <div>
                    <h3 style={{ margin: 0, fontSize: '22px', fontWeight: '700', color: 'white', letterSpacing: '-0.3px' }}>
                      {requestData.layout_type === 'library' ? 'Customize Selected Layout' : 'New Construction Request'}
                    </h3>
                    <p style={{ margin: '4px 0 0 0', fontSize: '14px', color: 'rgba(255,255,255,0.75)' }}>
                      {requestData.layout_type === 'library'
                        ? 'Provide your requirements to customize the selected layout'
                        : 'Fill in your project details to get matched with the right architects'}
                    </p>
                  </div>
                </div>
                {/* Progress indicator */}
                <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
                  {['Basic Info', 'Site Details', 'Preferences', 'Location', 'Architect'].map((label, i) => (
                    <div key={i} style={{ flex: 1, textAlign: 'center' }}>
                      <div style={{ height: '3px', borderRadius: '2px', background: i === 0 ? 'white' : 'rgba(255,255,255,0.3)', marginBottom: '4px' }} />
                      <span style={{ fontSize: '10px', color: i === 0 ? 'white' : 'rgba(255,255,255,0.5)', fontWeight: '500' }}>{label}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ padding: '0 32px 32px', maxHeight: 'calc(90vh - 180px)', overflowY: 'auto' }}>
                ...
                <div className="selected-layout-display">
                  ...
                  <div className="layout-preview">
                    <img
                      ...
                      onError={(e) => handleImageError(e, 'Layout preview not available')}
                      onLoad={handleImageLoad}
                    />
              ...
              <form onSubmit={handleRequestSubmit}>
                ...
                <div className="form-section" style={{ marginTop: '28px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', paddingBottom: '12px', borderBottom: '2px solid #f1f5f9' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #dbeafe, #bfdbfe)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>📐</div>
                    <h4 style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#1e3a5f' }}>Basic Information</h4>
                  </div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (value === '' || !isNaN(value)) {
                            setRequestData({ ...requestData, building_size: value });
                          }
                        }}
                        onKeyDown={(e) => {
                          // Prevent decrement with arrow down or other keys
                ...
                <div className="form-section" style={{ marginTop: '28px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', paddingBottom: '12px', borderBottom: '2px solid #f1f5f9' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #d1fae5, #a7f3d0)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>🏔️</div>
                    <h4 style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#1e3a5f' }}>Site Details</h4>
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                ...
                <div className="form-section" style={{ marginTop: '28px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', paddingBottom: '12px', borderBottom: '2px solid #f1f5f9' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #fce7f3, #fbcfe8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>🎨</div>
                    <h4 style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#1e3a5f' }}>Family & Design Preferences</h4>
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                ...
                <div className="form-section location-timeline-section" style={{ marginTop: '28px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', paddingBottom: '12px', borderBottom: '2px solid #f1f5f9' }}>
                    <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #fef3c7, #fde68a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>📍</div>
                    <h4 style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: '#1e3a5f' }}>Location & Timeline</h4>
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                      <label>State *</label>
                ...
                <div className="form-actions" style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', paddingTop: '8px', borderTop: '1px solid #f1f5f9', marginTop: '8px' }}>
                  <button
                    type="button"
                    onClick={() => {
                      setShowRequestForm(false);
                    ...
                    className="btn btn-secondary"
                    style={{ padding: '12px 24px', borderRadius: '10px', fontWeight: '600', fontSize: '15px' }}
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="btn btn-primary"
                    style={{ padding: '12px 28px', borderRadius: '10px', fontWeight: '600', fontSize: '15px', background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)', border: 'none', boxShadow: '0 4px 12px rgba(37,99,235,0.35)' }}
                  >
                    {loading ? '⏳ Submitting...' :
              ...
              <div className="form-row" style={{ gap: '16px' }}>
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  {isImageUrl(previewLayout.image_url) ? (
                    <SafeImage src={previewLayout.image_url} alt="Preview" fallbackText="Preview not available" style={{ width: '100%', maxHeight: '70vh', objectFit: 'contain', borderRadius: 8 }} onError={(e) => handleImageError(e, 'Preview image not available')} onLoad={handleImageLoad} />
                  ) : (
                ...
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  {isImageUrl(previewLayout.design_file_url) ? (
                    <SafeImage src={previewLayout.design_file_url} alt="Layout" fallbackText="Layout not available" style={{ width: '100%', maxHeight: '70vh', objectFit: 'contain', borderRadius: 8 }} onError={(e) => handleImageError(e, 'Layout image not available')} onLoad={handleImageLoad} />
                      ...
                      <div>
                        ...
                        <div style={{ marginTop: '8px' }}>
                          <SafeImage src={technicalDetailsModal.image_url} fallbackText="Technical details image not available"
                            ...
                            onError={(e) => handleImageError(e, 'Technical details image not available')}
                            onLoad={handleImageLoad}
                          />
                        </div>
                      </div>
                ...
                {isImage ? (
                  <div style={{ position: 'relative', width: '100%', height: 140 }}>
                    <img
                      ...
                      onError={(e) => handleImageError(e, 'File image not available')}
                      onLoad={handleImageLoad}
                    />
  ...
  return (
    <div className="list-item" style={{
      ...
      <div style={{
        ...
        {(request.location || request.district || request.panchayat_municipality) && (
          <div style={{
            background: '#f0fdf4',
            padding: '12px',
            borderRadius: '8px',
            ...
            <div style={{ fontSize: '14px', color: '#1f2937', fontWeight: '600', lineHeight: '1.5' }}>
              {[request.panchayat_municipality, request.district, request.location].filter(Boolean).join(', ')}
            </div>
          </div>
        )}
  ...
  return (
    <div className="list-item project-item">
      ...
      <div className="item-content">
        ...
        {project.construction_project_id && (
          <div style={{ marginTop: '12px' }}>
            <ProjectHealthWidget projectId={project.construction_project_id} role="homeowner" compact={false} />
          </div>
        )}
    ...
    <div className="layout-image-container">
      <button type="button" className="layout-image-button" onClick={(e) => { e.stopPropagation(); onPreview(); }} style={{ cursor: 'zoom-in' }}>
        <SafeImage 
          ...
          onError={(e) => {
            e.target.src = '/images/default-layout.jpg';
            if (e.target.src.includes('default-layout.jpg')) {
              e.target.style.display = 'none';
              const fallback = document.createElement('div');
              fallback.style.cssText = 'width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: #f5f5f7; flex-direction: column; color: #6b7280; font-size: 0.75rem;';
              fallback.innerHTML = '<span style="font-size: 2rem; margin-bottom: 8px;">🏠</span><span>Layout preview not available</span>';
          ...
          {isImage ? (
            <img
              ...
              onError={(e) => {
                e.target.style.display = 'none';
                const fallback = document.createElement('div');
                fallback.style.cssText = 'display: flex; align-items: center; justify-content: center; background: #f5f5f7; flex-direction: column; color: #6b7280; font-size: 1rem; text-align: center; padding: 2rem; border-radius: 8px; border: 1px dashed #d1d5db; min-height: 200px; width: 100%;';
                fallback.innerHTML = '<span style="font-size: 3rem; margin-bottom: 16px;">🖼️</span><span>Image could not be loaded</span>';
                e.target.parentNode.insertBefore(fallback, e.target);
              }}
            />
          ) : isPdf ? (
