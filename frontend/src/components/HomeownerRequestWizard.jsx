...
import RiskAssessmentPreview from './RiskAssessmentPreview.jsx';
import { keralaPanchayatsMunicipalities } from '../data/locationData';
import { getClimateProfile, climateToRiskModifiers } from '../data/climateData';
...
// Kerala-only location helper — returns places for a given district
const getKeralaDistrictPlaces = (district) => {
  if (!district || !keralaPanchayatsMunicipalities[district]) return [];
  const d = keralaPanchayatsMunicipalities[district];
  return [...(d.Municipalities || []), ...(d.Panchayats || [])].sort();
};

export default function HomeownerRequestWizard() {
  const navigate = useNavigate();
  const toast = useToast();
  const steps = ['Preliminary', 'Site', 'Family', 'Preferences', 'Review', 'Architect', 'Risk Assessment', 'Submit'];
  ...
  const [data, setData] = useState({
    ...
    kerala_district: '', construction_start_month: '',
  ...
  async function submit() {
    ...
    try {
      ...
      const submitData = {
        ...
        // Kerala ML fields
        kerala_district: data.kerala_district || '',
        construction_start_month: data.construction_start_month ? parseInt(data.construction_start_month) : null,
        // AI prediction from final review
  ...
  // Function to prepare data for risk assessment
  const prepareRiskAssessmentData = () => {
    const plotSizeInSqft = convertPlotSizeToSqft(data.plot_size, data.plot_unit);
    const buildingSizeInSqft = parseFloat(data.building_size) || 0;
    const budgetAmount = data.budget_range === 'Custom' ? 
      parseFloat(data.custom_budget) || 0 : 
      convertBudgetRangeToNumeric(data.budget_range);

    // Count rooms from floor_rooms data
    let totalBedrooms = 0;
    let totalBathrooms = 0;
    
    Object.values(data.floor_rooms || {}).forEach(floor => {
      totalBedrooms += parseInt(floor.bedrooms) || 0;
      totalBathrooms += parseInt(floor.bathrooms) || 0;
    });

    // Fallback to room_requirements if floor_rooms is empty
    if (totalBedrooms === 0 && totalBathrooms === 0) {
      totalBedrooms = parseInt(data.room_requirements?.bedrooms) || 2;
      totalBathrooms = parseInt(data.room_requirements?.bathrooms) || 1;
    }

    // Convert timeline string to numeric months (midpoint of range)
    const timelineToMonths = (timeline) => {
      const map = {
        '1-3 months':  2,
        '3-6 months':  4,
        '6-12 months': 9,
        '1-2 years':   18,
        '2+ years':    30,
        'Flexible':    0,  // 0 = let predictor estimate
      };
      return map[timeline] || 0;
    };

    return {
      plot_size_sqft: plotSizeInSqft,
      building_size_sqft: buildingSizeInSqft,
      num_floors: parseInt(data.num_floors) || 1,
      budget_amount: budgetAmount,
      num_bedrooms: totalBedrooms,
      num_bathrooms: totalBathrooms,
      planned_duration_months: timelineToMonths(data.timeline),
      plot_shape: data.plot_shape || 'rectangular',
      topography: data.topography || 'flat',
      design_style: 'modern',
      special_features: Array.isArray(data.family_needs) ? data.family_needs.join(', ') : data.family_needs || '',
      custom_requirements: data.requirements || '',
      architectural_preferences: Array.isArray(data.aesthetic) ? data.aesthetic.join(', ') : data.aesthetic || '',
      basement: false,
      terrace: false,
      parking: true,
      site_access_difficult: data.topography === 'steep_slope' || data.topography === 'hilly',
      utility_connections_needed: true,
      soil_issues: data.site_considerations?.toLowerCase().includes('soil') || false,
      remote_location: false,
      // Kerala-specific
      kerala_district: data.kerala_district || 'Ernakulam',
      construction_start_month: parseInt(data.construction_start_month) || 1,
      // Panchayat-level climate
      location: data.location || '',
      climate_profile: getClimateProfile(data.location, data.kerala_district),
      climate_modifiers: climateToRiskModifiers(getClimateProfile(data.location, data.kerala_district)),
    };
  };

  // Function to handle risk assessment step
  const handleRiskAssessment = () => {
    const riskData = prepareRiskAssessmentData();
    setRiskAssessmentData(riskData);
    setShowRiskAssessment(true);
  };

  // Function to proceed after risk assessment — go to final review step
  const handleRiskAssessmentProceed = (prediction) => {
    setShowRiskAssessment(false);
    setAiPrediction(prediction);
    setStep(7); // Go to final review/confirm step
  };

  // Function to revise after risk assessment
  const handleRiskAssessmentRevise = () => {
    setShowRiskAssessment(false);
    setStep(4); // Go back to Review step
  };

  // Helper function to convert plot size to sqft
  const convertPlotSizeToSqft = (size, unit) => {
    const numericSize = parseFloat(size) || 0;
    switch (unit) {
      case 'cents':
        return numericSize * 435.6; // 1 cent = 435.6 sqft
      case 'acres':
        return numericSize * 43560; // 1 acre = 43560 sqft
      case 'sqft':
      default:
        return numericSize;
    }
    ...
    >
        ...
        <div className="section">
          ...
          <div className="section-body" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            ...
            <div className="field">
              ...
              <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '12px' }}>
                Select the district where construction will happen
              </div>
              <select
                value={data.kerala_district}
                onChange={e => setData({ ...data, kerala_district: e.target.value, location: '' })}
                style={{ width: '100%', padding: '12px 16px', fontSize: '16px', border: '2px solid #e5e7eb', borderRadius: '8px' }}
              >
            ...
            {data.kerala_district && (
              <div className="field">
                <label style={{ fontSize: '18px', fontWeight: '600', color: '#1f2937', marginBottom: '8px', display: 'block' }}>
                  📍 Location in {data.kerala_district}
                </label>
                <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '12px' }}>
                  Select your panchayat or municipality
                </div>
                <SearchableDropdown
                  options={[
                    ...(keralaPanchayatsMunicipalities[data.kerala_district]?.Municipalities || []),
                    ...(keralaPanchayatsMunicipalities[data.kerala_district]?.Panchayats || []),
                  ].sort()}
                  value={data.location}
                  onChange={(value) => setData({ ...data, location: value })}
                  placeholder={`Search in ${data.kerala_district}...`}
            ...
            <div className="field">
              <label style={{ fontSize: '18px', fontWeight: '600', color: '#1f2937', marginBottom: '8px', display: 'block' }}>
                📅 Planned Construction Start Month
              </label>
              <div style={{ fontSize: '14px', color: '#6b7280', marginBottom: '12px' }}>
                When do you plan to start construction? (affects monsoon risk)
              </div>
              <select
                value={data.construction_start_month}
                onChange={e => setData({ ...data, construction_start_month: e.target.value })}
                style={{ width: '100%', padding: '12px 16px', fontSize: '16px', border: '2px solid #e5e7eb', borderRadius: '8px' }}
              >
                ...
                <option value="6">June 🌧️ SW Monsoon</option>
                <option value="7">July 🌧️ SW Monsoon</option>
                <option value="8">August 🌧️ SW Monsoon</option>
                <option value="9">September 🌧️ SW Monsoon</option>
                <option value="10">October 🌦️ NE Monsoon</option>
                <option value="11">November 🌦️ NE Monsoon</option>
                ...
                <div style={{ 
                  ...
                  <div style={{ fontSize: '14px', lineHeight: '1.8', color: '#374151' }}>
                    ...
                    <div style={{ marginBottom: '8px' }}>
                      ...
                      {data.location && (() => {
                        const cp = getClimateProfile(data.location, data.kerala_district);
                        const terrainIcon = { highland: '⛰️', hilly: '🏔️', midland: '🌄', coastal: '🌊', backwater: '🌿', flat: '🏞️' }[cp.terrain] || '📍';
                        const rainfallColor = { very_high: '#1d4ed8', high: '#2563eb', moderate: '#0891b2', low: '#d97706' }[cp.rainfall] || '#6b7280';
                        return (
                          <span style={{ marginLeft: '8px', fontSize: '12px', color: rainfallColor, backgroundColor: '#f0f9ff', padding: '2px 6px', borderRadius: '4px', border: `1px solid ${rainfallColor}30` }}>
                            {terrainIcon} {cp.terrain} · {cp.rainfall.replace('_', ' ')} rainfall
                            {cp.flood_risk === 'high' && ' · ⚠️ flood risk'}
                          </span>
                        );
                      })()}
                    </div>
                    <div style={{ marginBottom: '8px' }}>
                      <strong>Kerala District:</strong> {data.kerala_district || 'Not specified'}
                    </div>
                    <div style={{ marginBottom: '8px' }}>
                      <strong>Construction Start Month:</strong> {data.construction_start_month ? ['January','February','March','April','May','June','July','August','September','October','November','December'][parseInt(data.construction_start_month)-1] : 'Not specified'}
                    </div>
                    <div style={{ marginBottom: '8px' }}>
      ...
      {step === 6 && (
        <div className="section">
          ...
          <div className="section-body">
            <div className="risk-assessment-intro">
              <h3>🎯 AI-Powered Risk Assessment</h3>
              <p>Before submitting your request, let our AI analyze potential cost and time overrun risks based on your project details.</p>
              ...
              <div style={{
                ...
                <h4 style={{ color: '#1e40af', marginBottom: '8px' }}>🤖 How it works:</h4>
                <ul style={{ color: '#1e40af', fontSize: '14px', marginBottom: 0 }}>
                  <li>Machine Learning models analyze your project specifications</li>
                  <li>Trained on 1000+ construction projects with 94.7% and 98.9% accuracy</li>
                  <li>Provides explainable risk factors and recommendations</li>
                  <li>Helps you make informed decisions before proceeding</li>
                </ul>
              </div>
              ...
              <div className="project-summary" style={{
                ...
                <h4>📋 Project Summary for Assessment:</h4>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '14px' }}>
                  <div><strong>Plot Size:</strong> {data.plot_size} {data.plot_unit}</div>
                  <div><strong>Building Size:</strong> {data.building_size} sq ft</div>
                  <div><strong>Floors:</strong> {data.num_floors || 1}</div>
                  <div><strong>Budget:</strong> {data.budget_range === 'Custom' ? `₹${data.custom_budget}` : data.budget_range}</div>
                  <div><strong>Plot Shape:</strong> {data.plot_shape || 'Not specified'}</div>
                  <div><strong>Topography:</strong> {data.topography || 'Not specified'}</div>
                  <div><strong>District:</strong> {data.kerala_district || 'Not specified'}</div>
                  <div><strong>Start Month:</strong> {data.construction_start_month ? ['January','February','March','April','May','June','July','August','September','October','November','December'][parseInt(data.construction_start_month)-1] : 'Not specified'}</div>
                  {data.location && <div style={{ gridColumn: '1 / -1' }}><strong>Location:</strong> {data.location}
                    {(() => {
                      const cp = getClimateProfile(data.location, data.kerala_district);
                      return <span style={{ marginLeft: '8px', fontSize: '12px', color: '#0891b2' }}>({cp.terrain} · {cp.rainfall.replace('_', ' ')} rainfall{cp.flood_risk === 'high' ? ' · ⚠️ flood risk' : ''})</span>;
                    })()}
                  </div>}
                </div>
              </div>
          ...
          <div className="wizard-footer">
            ...
            <button 
              ...
              disabled={loading}
            >
              {loading ? 'Analyzing...' : 'Analyze Risks & Continue'}
            </button>
          </div>
        </div>
      )}
      ...
      {step === 7 && (
        <div className="section">
          <div className="section-header">Review &amp; Confirm</div>
          <div className="section-body">
            <div className="submit-message">
              <h3>Final Review</h3>
              <p>Check all your project details and the AI risk assessment below before submitting.</p>
            </div>

            {/* Project Details */}
            <div style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '20px', marginBottom: '16px' }}>
              <h4 style={{ margin: '0 0 16px 0', color: '#374151', fontSize: '16px' }}>📋 Project Details</h4>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px' }}>
                <div style={{ backgroundColor: 'white', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h5 style={{ margin: '0 0 12px 0', color: '#374151', fontSize: '14px' }}>🏠 Specifications</h5>
                  <div style={{ fontSize: '14px', color: '#6b7280', lineHeight: '1.8' }}>
                    ...
                    <div><strong>Bedrooms:</strong> {data.num_bedrooms}</div>
                    <div><strong>Bathrooms:</strong> {data.num_bathrooms}</div>
                    ...
                    {plotCategory && <div><strong>Plot Category:</strong> {plotCategory}</div>}
                  </div>
                </div>

                <div style={{ backgroundColor: 'white', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h5 style={{ margin: '0 0 12px 0', color: '#374151', fontSize: '14px' }}>👥 Selected Architects ({selectedArchitects.length})</h5>
                  <div style={{ fontSize: '14px', color: '#6b7280', lineHeight: '1.6' }}>
                    {selectedArchitects.length > 0 ? selectedArchitects.map((architect) => (
                      <div key={architect.id} style={{ padding: '8px 12px', backgroundColor: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: '6px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '32px', height: '32px', backgroundColor: '#3b82f6', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px', fontWeight: '600', flexShrink: 0 }}>
                          {architect.first_name?.charAt(0) || 'A'}{architect.last_name?.charAt(0) || ''}
                        </div>
                        <div>
                          <div style={{ fontWeight: '600', color: '#1e40af' }}>{architect.first_name} {architect.last_name}</div>
                          <div style={{ fontSize: '12px', color: '#6b7280' }}>{architect.email}</div>
                          {architect.specialization && <div style={{ fontSize: '12px', color: '#059669' }}>{architect.specialization}</div>}
                        </div>
                      </div>
                    )) : (
                      <div style={{ color: '#9ca3af', fontStyle: 'italic' }}>No architects selected</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            {aiPrediction && (() => {
              const costRisk = aiPrediction.cost_overrun_risk;
              const timeRisk = aiPrediction.time_delay_risk;
              const riskScore = aiPrediction.risk_score;
              const finalRisk = aiPrediction.final_risk;
              const derived = aiPrediction.derived_metrics;
              const confidence = aiPrediction.ml_confidence;
              ...
              return (
                <div style={{ backgroundColor: riskBg[finalRisk] || '#f8fafc', border: `1px solid ${riskBorder[finalRisk] || '#e2e8f0'}`, borderRadius: '8px', padding: '20px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
                    <h4 style={{ margin: 0, color: '#374151', fontSize: '16px' }}>🤖 AI Risk Assessment Summary</h4>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <span style={{ fontSize: '13px', color: '#6b7280' }}>Overall Risk Score: <strong style={{ color: riskColor[finalRisk] }}>{riskScore}/100</strong></span>
                      <span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '13px', fontWeight: '700', backgroundColor: riskColor[finalRisk], color: 'white' }}>{finalRisk}</span>
                    </div>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
                    {/* Cost Risk */}
                    <div style={{ backgroundColor: 'white', padding: '14px', borderRadius: '8px', border: `1px solid ${riskBorder[costRisk?.risk_level] || '#e2e8f0'}` }}>
                      <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>💰 Budget Risk</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                        <span style={{ fontWeight: '700', fontSize: '16px', color: riskColor[costRisk?.risk_level] }}>{costRisk?.risk_level}</span>
                        <span style={{ fontSize: '12px', color: '#9ca3af' }}>score: {costRisk?.rule_score}</span>
                      </div>
                      {derived && (
                        <div style={{ fontSize: '12px', color: '#6b7280' }}>
                          ₹{Math.round(derived.budget_per_sqft).toLocaleString('en-IN')}/sqft
                        </div>
                      )}
                      {costRisk?.reason && <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px', fontStyle: 'italic' }}>{costRisk.reason}</div>}
                    </div>

                    {/* Time Risk */}
                    <div style={{ backgroundColor: 'white', padding: '14px', borderRadius: '8px', border: `1px solid ${riskBorder[timeRisk?.risk_level] || '#e2e8f0'}` }}>
                      <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>⏱ Timeline Risk</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                        <span style={{ fontWeight: '700', fontSize: '16px', color: riskColor[timeRisk?.risk_level] }}>{timeRisk?.risk_level}</span>
                        <span style={{ fontSize: '12px', color: '#9ca3af' }}>score: {timeRisk?.rule_score}</span>
                      </div>
                      {timeRisk && (
                        <div style={{ fontSize: '12px', color: '#6b7280' }}>
                          Planned: {timeRisk.planned_duration} mo · Expected: {timeRisk.expected_duration} mo
                        </div>
                      )}
                      {timeRisk?.reason && <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px', fontStyle: 'italic' }}>{timeRisk.reason}</div>}
                    </div>

                    {/* Confidence */}
                    <div style={{ backgroundColor: 'white', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                      <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>📊 AI Confidence</div>
                      <div style={{ fontSize: '14px', color: '#374151', lineHeight: '1.8' }}>
                        <div>Cost: <strong>{confidence ? Math.round(confidence.cost_max_prob * 100) : '—'}%</strong></div>
                        <div>Time: <strong>{confidence ? Math.round(confidence.time_max_prob * 100) : '—'}%</strong></div>
                        <div style={{ fontSize: '12px', color: confidence?.ml_available ? '#16a34a' : '#d97706', marginTop: '4px' }}>
                          {confidence?.ml_available ? '✓ ML model active' : '⚠ Rule-based estimate'}
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* AI Predicted Estimates */}
                  {derived && (function() {
                    const buildingSqft = parseFloat(data.building_size) || 0;
                    const bps = derived.budget_per_sqft || 0;
                    const baseCost = Math.round(bps * buildingSqft);
                    const overrunPct = finalRisk === 'High' ? 0.25 : finalRisk === 'Low' ? 0.05 : 0.15;
                    const predictedCost = Math.round(baseCost * (1 + overrunPct));
                    const expectedDur = derived.expected_duration || (timeRisk && timeRisk.expected_duration) || 0;
                    const monsoonBuf = derived.monsoon_exposure > 0.3 ? Math.round(expectedDur * 0.15) : 0;
                    const predictedDur = Math.round(expectedDur + monsoonBuf);
                    const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
                    const startM = parseInt(data.construction_start_month) || 1;
                    const endMonthName = monthNames[((startM - 1 + predictedDur) % 12)];
                    const endYear = new Date().getFullYear() + Math.floor((startM - 1 + predictedDur) / 12);
                    return (
                      <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#f0f9ff', borderRadius: '8px', border: '1px solid #bae6fd' }}>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: '#1e40af', marginBottom: '12px' }}>🤖 AI Predicted Estimates</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #e0f2fe' }}>
                            <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Predicted Total Cost</div>
                            <div style={{ fontSize: '20px', fontWeight: '700', color: '#1e40af' }}>
                              {predictedCost > 0 ? '₹' + predictedCost.toLocaleString('en-IN') : '—'}
                            </div>
                            {baseCost > 0 && <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px' }}>Base ₹{baseCost.toLocaleString('en-IN')} + {Math.round(overrunPct * 100)}% {finalRisk.toLowerCase()} risk buffer</div>}
                          </div>
                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #e0f2fe' }}>
                            <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Predicted Duration</div>
                            <div style={{ fontSize: '20px', fontWeight: '700', color: '#1e40af' }}>
                              {predictedDur > 0 ? predictedDur + ' months' : '—'}
                            </div>
                            {monsoonBuf > 0 && <div style={{ fontSize: '11px', color: '#d97706', marginTop: '2px' }}>Includes +{monsoonBuf} mo monsoon buffer</div>}
                            {predictedDur > 0 && <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px' }}>Est. completion: {endMonthName} {endYear}</div>}
                          </div>
                          <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #e0f2fe' }}>
                            <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Cost per sq ft</div>
                            <div style={{ fontSize: '20px', fontWeight: '700', color: '#1e40af' }}>
                              {bps > 0 ? '₹' + Math.round(bps).toLocaleString('en-IN') + '/sqft' : '—'}
                            </div>
                            <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px' }}>For {buildingSqft.toLocaleString('en-IN')} sq ft</div>
                          </div>
                        </div>
                        <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '10px', fontStyle: 'italic' }}>* AI estimates based on Kerala construction benchmarks. Actual costs may vary.</div>
                        {/* Panchayat climate context */}
                        {(derived.location || derived.terrain || derived.rainfall) && (
                          <div style={{ marginTop: '10px', padding: '8px 12px', backgroundColor: '#ecfdf5', borderRadius: '6px', border: '1px solid #a7f3d0', fontSize: '12px', color: '#065f46' }}>
                            📍 <strong>{derived.location || data.location || data.kerala_district}</strong>
                            {derived.terrain && <span> · {derived.terrain} terrain</span>}
                            {derived.rainfall && <span> · {derived.rainfall.replace('_', ' ')} rainfall zone</span>}
                            {derived.flood_risk && derived.flood_risk !== 'low' && <span> · ⚠️ {derived.flood_risk} flood risk</span>}
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {aiPrediction.risk_reduction_suggestions?.length > 0 && (
                    <div style={{ marginTop: '12px', padding: '12px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                      <div style={{ fontSize: '13px', color: '#374151', fontWeight: '600', marginBottom: '8px' }}>💡 Top Recommendations</div>
                      {aiPrediction.risk_reduction_suggestions.slice(0, 2).map((s, i) => (
                        <div key={i} style={{ fontSize: '13px', color: '#6b7280', padding: '6px 0', borderBottom: i === 0 && aiPrediction.risk_reduction_suggestions.length > 1 ? '1px solid #f3f4f6' : 'none' }}>
                          {s.suggestion || s.description || s}
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="wizard-footer">
            <button className="btn btn-secondary" onClick={() => setStep(6)}>← Back to AI Analysis</button>
            <button className="btn btn-primary" onClick={submit}>Confirm &amp; Submit Request</button>
          </div>
        </div>
      )}
      ...
      <RiskAssessmentPreview
        ...
        onProceed={handleRiskAssessmentProceed}
        onRevise={handleRiskAssessmentRevise}
      />
    </WizardLayout>
  );
}
