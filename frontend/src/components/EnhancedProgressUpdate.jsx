...
import ContractorSubmittedReports from './ContractorSubmittedReports.jsx';
...
const EnhancedProgressUpdate = ({ contractorId, onUpdateSubmitted }) => {
  ...
  console.log(`🔄 EnhancedProgressUpdate RENDER #${renderCountRef.current}`);
  ...
  const [activeSection, setActiveSection] = useState('daily'); // 'daily', 'reports', 'weekly', 'monthly'
  ...
  const validateForm = (formType) => {
    ...
    if (formType === 'daily') {
      ...
      // Stage progression validation - make it more helpful
      if (dailyForm.construction_stage && dailyForm.incremental_completion_percentage && stageBreakdown) {
        ...
        if (!stageValidation.isValid) {
          ...
          const remaining = stage ? Math.max(0, stage.percentage - stageData.current_progress) : 0;
          
          if (remaining <= 0) {
            errors.stage_progression = `${dailyForm.construction_stage} stage is complete! Please select the next stage.`;
          } else {
            errors.stage_progression = `Maximum ${remaining.toFixed(1)}% allowed for ${dailyForm.construction_stage} stage. Use the helper buttons below to set the correct amount.`;
          }
        }
        
        // Show warnings as info, not errors
        if (stageValidation.warnings && stageValidation.warnings.length > 0) {
          // Don't treat warnings as blocking errors
          console.log('Stage warnings:', stageValidation.warnings);
        }
      }
  ...
  // MUST be defined BEFORE any hooks or useMemo that use it to avoid temporal dead zone error
  ...
  // Auto-select next available stage when current is completed
  useEffect(() => {
    if (dailyForm.construction_stage && stageBreakdown) {
      const stage = CONSTRUCTION_STAGES.find(s => s.name === dailyForm.construction_stage);
      const stageData = stageBreakdown.stages[dailyForm.construction_stage];
      
      // If current stage is completed, automatically switch to next stage
      if (stageData && stageData.is_completed) {
        const availableStages = getAvailableStages(stageBreakdown);
        const nextStage = availableStages.find(s => s.name !== dailyForm.construction_stage && !s.is_completed);
        
        if (nextStage) {
          toast.success(`🎉 ${dailyForm.construction_stage} stage completed! Automatically switched to ${nextStage.name} stage.`);
          setDailyForm(prev => ({
            ...prev,
            construction_stage: nextStage.name,
            incremental_completion_percentage: ''
          }));
        } else {
          // All stages completed
          toast.success(`🎉 ${dailyForm.construction_stage} stage completed! All construction stages are now finished!`);
          setDailyForm(prev => ({
            ...prev,
            construction_stage: '',
            incremental_completion_percentage: ''
  ...
  }, [stageBreakdown, dailyForm.construction_stage]);
  ...
  const handleDailyInputChange = (e) => {
    ...
    console.log(`Input change - Field: ${name}, Value: "${value}", Type: ${typeof value}`);
    
    // For percentage fields, apply smart capping and validation
    let finalValue = value;
    if (name === 'incremental_completion_percentage') {
      // Parse the input value
      const numericValue = parseFloat(value);
      
      // If we have stage breakdown data, cap the value to remaining percentage
      if (dailyForm.construction_stage && stageBreakdown && !isNaN(numericValue)) {
        const stage = CONSTRUCTION_STAGES.find(s => s.name === dailyForm.construction_stage);
        const stageData = stageBreakdown.stages[dailyForm.construction_stage];
        const remaining = stage ? stage.percentage - stageData.current_progress : 0;
        
        // Cap the value to remaining percentage (with small tolerance)
        if (numericValue > remaining + 0.1) {
          finalValue = remaining.toFixed(1);
          
          // Show a helpful toast message
          toast.info(`Progress capped to ${remaining.toFixed(1)}% (remaining in ${dailyForm.construction_stage} stage)`);
        }
      }
      ...
      if (!isNaN(numericValue) && numericValue > 100) {
        finalValue = '100';
        toast.warning('Daily progress cannot exceed 100%');
      }
    }
    ...
    setDailyForm(prev => {
      ...
      console.log(`Form updated - ${name}: "${newForm[name]}"`);
      return newForm;
    });
    
    // Real-time validation (but don't let it modify the value)
    handleFieldValidation(name, finalValue, { ...dailyForm, [name]: finalValue });
  };
  ...
  // Memoize selected project info to ensure it updates when selectedProject changes
  const selectedProjectInfo = React.useMemo(() => {
    console.log('Computing selectedProjectInfo for selectedProject:', selectedProject);
    console.log('Available projects:', projects.map(p => ({ id: p.id, project_id: p.project_id, name: p.project_name })));
    ...
    // Find the exact project by ID - use strict matching to avoid wrong project selection
    const project = projects.find(p => (p.id || p.project_id) == selectedProject);
    console.log('Found project by ID match:', project ? { id: project.id, name: project.project_name } : 'NOT FOUND');
    
    // Return the exact project found - don't try to merge with other projects
    // This ensures we show the correct project details for the selected project
    return project || null;
  ...
  return (
    <div className="enhanced-progress-update">
          ...
          <div className="project-selection-container">
            <select
              ...
              onChange={(e) => {
                ...
                console.log('Calling setSelectedProject with:', newProjectId);
                setSelectedProject(newProjectId);
                console.log('setSelectedProject called');
                handleFieldValidation('selectedProject', newProjectId);
              }}
      ...
      {selectedProject && (
        <div className="selected-project-info-card" key={`project-info-${selectedProject}`}>
          {(() => {
            const projectInfo = selectedProjectInfo;
            console.log('Rendering project info for selectedProject:', selectedProject, 'projectInfo:', projectInfo);
            if (!projectInfo) {
              console.warn('No project info found for selectedProject:', selectedProject);
              return <div className="no-project-info">Project information not available</div>;
            }
            ...
            <div className={`form-group ${getFieldErrorClass('construction_stage')}`}>
              ...
              >
                <option value="">
                  {availableStages.length === 0 
                    ? 'All stages completed - project finished!' 
                    : 'Select stage...'
                  }
                </option>
                {availableStages
                  .filter(stage => !stage.is_completed) // Only show incomplete stages
                  .map(stage => {
                    const remaining = stage.remaining_percentage || 0;
                    const statusText = stage.is_current ? ' (Current)' : '';
                    const remainingText = remaining > 0 ? ` - ${remaining.toFixed(1)}% remaining` : ' - Ready to start';
                    
                    return (
                      <option key={stage.name} value={stage.name}>
                        {stage.icon} {stage.name}{statusText}{remainingText}
                      </option>
                    );
                  })}
              </select>
              ...
              {progressSummary && (
                <div className="stage-progression-info">
                  ...
                  {progressSummary.completed_stages > 0 && (
                    <div className="completed-stages-info">
                      <span className="completed-stages">
                        ✅ Completed: {progressSummary.stage_details
                  ...
                  <div className="current-stage-info">
                    {progressSummary.current_stage ? (
                      ...
                      <span className="ready-stage">
                        ⏳ Ready to start: {availableStages[0]?.icon} {availableStages[0]?.name}
                      </span>
                    ) : (
                      <span className="project-complete">
                  ...
                  {/* Show next stage */}
                  {progressSummary.next_stage && availableStages.length > 1 && (
            ...
            <div className={`form-group ${getFieldErrorClass('incremental_completion_percentage')}`}>
                ...
                {dailyForm.construction_stage && stageBreakdown && (
                  <span className="stage-limit-info">
                    {(() => {
                      ...
                      const stageData = stageBreakdown.stages[dailyForm.construction_stage];
                      const remaining = stage ? Math.max(0, stage.percentage - stageData.current_progress) : 0;
                      const current = stageData.current_progress || 0;
                      
                      if (remaining <= 0) {
                        return '(Stage Complete ✅)';
                      } else if (remaining < 12.5) {
                        return `(${remaining.toFixed(1)}% remaining to complete stage)`;
                      } else {
                        return `(Max: ${remaining.toFixed(1)}% for this stage)`;
                      }
                    })()}
                  </span>
                )}
              ...
              <div className="percentage-input">
                ...
                {dailyForm.construction_stage && stageBreakdown && (() => {
                  ...
                  return (
                    <div className="progress-helper-buttons">
                      {remaining > 0 && (
                        <>
                          {/* Complete stage button */}
                          <button
                            type="button"
                            className="helper-btn complete-stage"
                            onClick={() => {
                              setDailyForm(prev => ({
                                ...prev,
                                incremental_completion_percentage: remaining.toFixed(1)
                              }));
                              toast.success(`Set to complete ${dailyForm.construction_stage} stage (${remaining.toFixed(1)}%)`);
                            }}
                            title={`Complete ${dailyForm.construction_stage} stage with remaining ${remaining.toFixed(1)}%`}
                          >
                            Complete Stage ({remaining.toFixed(1)}%)
                          </button>
                          
                          {/* Half progress button */}
                          {remaining >= 2 && (
                            <button
                              type="button"
                              className="helper-btn half-progress"
                              onClick={() => {
                                const halfRemaining = (remaining / 2).toFixed(1);
                                setDailyForm(prev => ({
                                  ...prev,
                                  incremental_completion_percentage: halfRemaining
                                }));
                                toast.info(`Set to half of remaining progress (${halfRemaining}%)`);
                              }}
                              title={`Add half of remaining progress`}
                            >
                              Half ({(remaining / 2).toFixed(1)}%)
                            </button>
                          )}
                          
                          {/* Small progress button */}
                          <button
                            type="button"
                            className="helper-btn small-progress"
                            onClick={() => {
                              const smallProgress = Math.min(2, remaining).toFixed(1);
                              setDailyForm(prev => ({
                                ...prev,
                                incremental_completion_percentage: smallProgress
                              }));
                              toast.info(`Set to small daily progress (${smallProgress}%)`);
                            }}
                            title="Add small daily progress"
                          >
                            Small Progress ({Math.min(2, remaining).toFixed(1)}%)
                          </button>
                        </>
                      )}
                      ...
                      <button
                        type="button"
                        className="helper-btn clear-progress"
                        onClick={() => {
                          setDailyForm(prev => ({
                            ...prev,
                            incremental_completion_percentage: ''
                        ...
                        title="Clear progress percentage"
                      >
                        Clear
                      </button>
                    </div>
      ...
      {activeSection === 'reports' && (
        <div className="progress-section reports-section">
          ...
          <div className={`form-group ${getFieldErrorClass('stages_worked')}`}>
            ...
            <div className="checkbox-grid">
              {CONSTRUCTION_STAGES.map(stage => (
                <label key={stage.name} className="checkbox-item">
                  <input
                    type="checkbox"
                    checked={weeklyForm.stages_worked.includes(stage.name)}
