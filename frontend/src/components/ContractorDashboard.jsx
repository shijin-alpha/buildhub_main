...
import StageDocumentManager from './StageDocumentManager.jsx';
...
const ContractorDashboard = () => {
  ...
  // Overrun data for completed projects
  ...
  const populateFormFromDraft = (formEl, draftData) => {
    ...
    // Force a recalculation after loading draft
    try {
      if (typeof recalcTotalsFromForm === 'function') {
        recalcTotalsFromForm(formEl);
      }
    } catch (e) {
      console.error('Error recalculating from draft:', e);
    }
  };
      ...
      try {
        ...
        if (me?.id) {
          ...
          if (mounted && j5?.success) {
            ...
            // Fetch overrun data for completed projects
            const completedProjects = (j5.data.projects || []).filter(p => {
              const progress = parseFloat(p.project_summary?.progress || '0');
              return progress >= 100 || p.project_summary?.status === 'completed';
            });
            
            // Fetch overrun data for each completed project
            completedProjects.forEach(async (project) => {
              try {
                const overrunRes = await fetch(
                  `/buildhub/backend/api/contractor/get_completed_project_overruns.php?contractor_id=${me.id}&project_id=${project.id}`,
                  { credentials: 'include' }
                );
                const overrunData = await overrunRes.json();
                if (overrunData.success) {
                  setProjectOverruns(prev => ({
                    ...prev,
                    [project.id]: overrunData.data
                  }));
                }
              } catch (err) {
                console.error(`Failed to fetch overrun data for project ${project.id}:`, err);
              }
            });
          }
                ...
                {ackOpenById[item.id] && (
                  <div className="acknowledgment-form">
                    ...
                    <button 
                      ...
                      // INSTANT CONFIRMATION - Update UI immediately
                      ...
                      toast.success(`✅ Acknowledgement sent! Acknowledged at: ${ackTime}, Due date: ${dueDate}. The homeowner has been notified.`);
                      
                      // Optimistically update the inbox item
                      setInbox(prevInbox => prevInbox.map(inboxItem => 
                        inboxItem.id === item.id 
                          ? { ...inboxItem, acknowledged_at: new Date().toISOString(), due_date: ackDateById[item.id] || null }
                          : inboxItem
                      ));
                      ...
                      try {
                        ...
                        if (!result.success) {
                          ...
                          setInbox(prevInbox => prevInbox.map(inboxItem => 
                            inboxItem.id === item.id 
                              ? { ...inboxItem, acknowledged_at: null, due_date: null }
                              : inboxItem
                          ));
                        }
                      } catch (e) {
                      ...
                      // Refresh inbox in background to sync with server
                      try {
                        const r = await fetch(`/buildhub/backend/api/contractor/get_inbox.php?contractor_id=${me.id}`, { credentials: 'include' });
                        const j = await r.json().catch(() => ({}));
                        if (j?.success) setInbox(Array.isArray(j.items) ? j.items : []);
          ...
          <div style={{display:'grid',gap:6}}>
            ...
            {item.acknowledged_at ? (
              ...
              <form 
                ...
                {/* AI Estimate Assistant - Smart Suggestions */}
                {(item.ai_predictions || payload.ai_predictions) && (() => {
                  const predictions = item.ai_predictions || payload.ai_predictions || {};
                  const costRisk = predictions.cost_risk_level || predictions.predicted_cost_risk_level;
                  const costProb = predictions.cost_probability || predictions.predicted_cost_probability;
                  ...
                  // Calculate suggested amounts based on risk
                  const baseBudget = item?.budget_range || item?.layout_request_details?.budget_range || '30-50 Lakhs';
                  const budgetMatch = baseBudget.match(/(\d+)/);
                  const baseCost = budgetMatch ? parseInt(budgetMatch[0]) * 100000 : 3500000;
                  
                  let contingencyPercent = 5;
                  if (costRisk === 'High') contingencyPercent = 18;
                  else if (costRisk === 'Medium') contingencyPercent = 10;
                  
                  const contingencyAmount = Math.round(baseCost * (contingencyPercent / 100));
                  const suggestedCost = baseCost + contingencyAmount;
                  
                  const baseTimeline = item?.timeline || item?.layout_request_details?.timeline || '6-12 months';
                  const timelineMatch = baseTimeline.match(/(\d+)/);
                  const baseMonths = timelineMatch ? parseInt(timelineMatch[0]) : 6;
                  
                  let bufferWeeks = 0;
                  if (timeRisk === 'High') bufferWeeks = 3;
                  else if (timeRisk === 'Medium') bufferWeeks = 2;
                  
                  const suggestedTimeline = bufferWeeks > 0 
                    ? `${baseMonths} months + ${bufferWeeks} weeks`
                    : `${baseMonths} months`;
                  ...
                  const applyAISuggestions = () => {
                    ...
                    if (totalCostInput) {
                      totalCostInput.value = suggestedCost.toString();
                      totalCostInput.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                    if (timelineInput) {
                      timelineInput.value = suggestedTimeline;
                      timelineInput.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                    
                    if (notesTextarea) {
                      const aiNote = `AI Risk Assessment: This project has ${costRisk.toLowerCase()} cost risk (${(costProb * 100).toFixed(1)}%) and ${timeRisk.toLowerCase()} time risk (${(timeProb * 100).toFixed(1)}%). A ${contingencyPercent}% contingency (${formatCurrency(contingencyAmount)}) has been included to cover potential overruns. ${bufferWeeks > 0 ? `Timeline includes ${bufferWeeks}-week buffer for potential delays.` : ''}`;
                      notesTextarea.value = aiNote;
                      notesTextarea.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                    
                    // Trigger form change to recalculate and save draft
                    ...
                    try {
                      ...
                      handleFormChange(item?.id, form);
                    } catch (e) {
                      console.error('Error in form update:', e);
                    }
                    
                    toast.success('✨ AI suggestions applied to estimate form!');
                  };
                      ...
                      <div style={{ paddingLeft: '34px' }}>
                        ...
                        <div style={{
                          ...
                          <div style={{ marginBottom: '12px' }}>
                            ...
                            <div style={{ paddingLeft: '20px', fontSize: '13px', color: '#374151' }}>
                              <div style={{ marginBottom: '4px' }}>
                                Base estimate: <strong>{formatCurrency(baseCost)}</strong>
                              </div>
                              <div style={{ marginBottom: '4px', color: '#f59e0b' }}>
                                + {contingencyPercent}% contingency: <strong>{formatCurrency(contingencyAmount)}</strong>
                              </div>
                              <div style={{
                                fontSize: '16px',
                                fontWeight: '700',
                          ...
                          <div style={{ marginBottom: '12px' }}>
                            ...
                            <div style={{ paddingLeft: '20px', fontSize: '13px', color: '#374151' }}>
                              <div style={{ marginBottom: '4px' }}>
                                Base timeline: <strong>{baseMonths} months</strong>
                              </div>
                              {bufferWeeks > 0 && (
                                <div style={{ marginBottom: '4px', color: '#f59e0b' }}>
                                  + {bufferWeeks} weeks buffer
                                </div>
                              )}
                              <div style={{
                                fontSize: '16px',
                          ...
                          <div style={{
                            ...
                            <div style={{ marginTop: '6px', lineHeight: '1.6' }}>
                              {costRisk === 'High' && 'High cost risk detected. Adding 15-20% contingency protects you from unexpected expenses. '}
                              {costRisk === 'Medium' && 'Medium cost risk detected. Adding 10% contingency provides reasonable protection. '}
                              {costRisk === 'Low' && 'Low cost risk detected. Minimal contingency needed. '}
                              {timeRisk === 'High' && 'High time risk suggests adding 3-week buffer to meet deadlines. '}
                              {timeRisk === 'Medium' && 'Medium time risk suggests adding 2-week buffer for safety. '}
                              {timeRisk === 'Low' && 'Low time risk allows standard timeline. '}
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{
                          ...
                          💡 Tip: You can still adjust the amounts after applying AI suggestions
                        </div>
                      </div>
                    </div>
                  );
                ...
                {/* AI Prediction Reference Panel */}
                {(() => {
                  // Backend now always resolves ai_predictions at the top level.
                  // payload.ai_predictions is the new-flow source; item.ai_predictions is the
                  // resolved value (covers both new flow and old projects via DB fallback).
                  const aiPred = item?.ai_predictions || item?.payload?.ai_predictions;
                  const predData = aiPred?.prediction_data;
                  if (!aiPred || (!aiPred.cost_risk_level && !aiPred.time_risk_level && !predData)) return null;

                  const costRisk = aiPred.cost_risk_level || predData?.cost_overrun_risk?.risk_level;
                  const timeRisk = aiPred.time_risk_level || predData?.time_delay_risk?.risk_level;
                  const riskScore = aiPred.risk_score || predData?.risk_score;
                  const derived = predData?.derived_metrics;
                  const timeDetails = predData?.time_delay_risk;
                  ...
                  const buildingSqft = parseFloat(item?.payload?.building_size || item?.payload?.layout_request_details?.building_size) || 0;
                  const bps = derived?.budget_per_sqft || 0;
                  const baseCost = Math.round(bps * buildingSqft);
                  const overrunPct = costRisk === 'High' ? 0.25 : costRisk === 'Low' ? 0.05 : 0.15;
                  const predictedCost = baseCost > 0 ? Math.round(baseCost * (1 + overrunPct)) : null;

                  // Compute predicted duration
                  const expectedDur = derived?.expected_duration || timeDetails?.expected_duration || 0;
                  const monsoonBuf = derived?.monsoon_exposure > 0.3 ? Math.round(expectedDur * 0.15) : 0;
                  const predictedDur = expectedDur > 0 ? Math.round(expectedDur + monsoonBuf) : null;

                  return (
                    <div style={{margin:'12px 0', padding:'14px 16px', background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:8}}>
                      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10, flexWrap:'wrap', gap:6}}>
                        <span style={{fontWeight:700, fontSize:13, color:'#1e40af'}}>🤖 AI Predicted Reference</span>
                        {riskScore != null && (
                          <span style={{fontSize:12, color:'#6b7280'}}>
                            Risk Score: <strong style={{color: riskColor[costRisk] || '#374151'}}>{riskScore}/100</strong>
                            {costRisk && <span style={{marginLeft:8, padding:'2px 8px', borderRadius:12, fontSize:11, fontWeight:700, background: riskColor[costRisk], color:'white'}}>{costRisk}</span>}
                          </span>
                        )}
                      </div>
                      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
                        <div style={{background:'white', padding:'10px 12px', borderRadius:6, border:`1px solid ${riskBorder[costRisk] || '#e2e8f0'}`}}>
                          <div style={{fontSize:11, color:'#6b7280', marginBottom:4, textTransform:'uppercase', letterSpacing:'0.04em'}}>💰 AI Predicted Cost</div>
                          <div style={{fontSize:18, fontWeight:700, color: riskColor[costRisk] || '#1e40af'}}>
                            {predictedCost ? '₹' + predictedCost.toLocaleString('en-IN') : (costRisk ? '—' : '—')}
                          </div>
                          {baseCost > 0 && <div style={{fontSize:11, color:'#6b7280', marginTop:2}}>Base ₹{baseCost.toLocaleString('en-IN')} + {Math.round(overrunPct*100)}% {costRisk?.toLowerCase()} risk buffer</div>}
                          {bps > 0 && <div style={{fontSize:11, color:'#6b7280'}}>₹{Math.round(bps).toLocaleString('en-IN')}/sqft</div>}
                          <div style={{fontSize:11, color:'#9ca3af', marginTop:4, fontStyle:'italic'}}>Budget risk: {costRisk || '—'}</div>
                        </div>
                        <div style={{background:'white', padding:'10px 12px', borderRadius:6, border:`1px solid ${riskBorder[timeRisk] || '#e2e8f0'}`}}>
                          <div style={{fontSize:11, color:'#6b7280', marginBottom:4, textTransform:'uppercase', letterSpacing:'0.04em'}}>⏱ AI Predicted Duration</div>
                          <div style={{fontSize:18, fontWeight:700, color: riskColor[timeRisk] || '#1e40af'}}>
                            {predictedDur ? predictedDur + ' months' : '—'}
                          </div>
                          {monsoonBuf > 0 && <div style={{fontSize:11, color:'#d97706', marginTop:2}}>Includes +{monsoonBuf} mo monsoon buffer</div>}
                          {timeDetails?.planned_duration && <div style={{fontSize:11, color:'#6b7280'}}>Planned: {timeDetails.planned_duration} mo · Expected: {timeDetails.expected_duration} mo</div>}
                          <div style={{fontSize:11, color:'#9ca3af', marginTop:4, fontStyle:'italic'}}>Timeline risk: {timeRisk || '—'}</div>
                        </div>
                      </div>
                      <div style={{fontSize:11, color:'#6b7280', marginTop:8, fontStyle:'italic'}}>* AI estimates for reference only. Your actual estimate may differ based on site conditions.</div>
                    </div>
                  );
                })()}
                ...
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8}}>
                  <div>
                    ...
                    <input name="total_cost" type="number" step="0.01" placeholder="Auto-calculated or enter manually" required style={{width:'100%', border:'1px solid #e5e7eb', borderRadius:6, padding:8, background:'#fff'}} />
                  </div>
                  <div>
        ...
        <div className="section-content">
          {loading ? (
            ...
            <div className="empty-state">
                ...
                <p>All your projects are completed! Check the Completed Projects section below.</p>
            </div>
          ) : (
                  ...
                  <div key={project.id} className="construction-project-card">
                    ...
                    {expandedProject === project.id && (
                      <div className="project-details">
                        {/* Unified Project Health Panel - contractor view */}
                        {project.id && (
                          <div style={{ marginBottom: '16px' }}>
                            <ProjectHealthWidget projectId={project.id} role="contractor" compact={false} />
                          </div>
                        )}
      ...
      {completedProjects.length > 0 && (
        <div className="section-card" style={{marginTop: '24px'}}>
          <div className="section-header">
            ...
            <p>Successfully finished construction projects</p>
            <div className="section-actions" style={{display: 'flex', gap: '12px', marginTop: '12px'}}>
              <div style={{
                padding: '8px 16px',
                background: 'linear-gradient(135deg, #10b981, #059669)',
          ...
          <div className="section-content">
            <div className="construction-projects">
              {completedProjects.map(project => (
                <div key={project.id} className="construction-project-card" style={{
                  border: '2px solid #10b981',
                  ...
                  <div className="project-header">
                    ...
                    <div className="project-title-section">
                      ...
                      <div className="project-meta">
                        ...
                        <span className="meta-item">Completed: {project.project_summary.last_activity}</span>
                      </div>
                    </div>
                    <div className="project-actions" style={{display: 'flex', gap: '8px', flexWrap: 'wrap'}}>
                      <button 
                      ...
                      <button 
                        className="btn btn-secondary"
                        onClick={() => {
                          const subject = `Project Completion - ${project.project_name}`;
                          const body = `Hi ${project.homeowner_name},\n\nI'm pleased to inform you that your construction project has been completed!\n\nProject: ${project.project_name}\nTotal Cost: ${project.technical_summary.total_cost}\nCompletion: ${project.project_summary.progress}\nFinal Stage: ${project.project_summary.current_stage}\n\nThank you for choosing us for your construction needs. We hope you're satisfied with the work!\n\nBest regards,\n[Your Name]`;
                          window.open(`mailto:${project.homeowner_email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`);
                        }}
                        title="Send completion email to homeowner"
                      >
                        📧 Contact Homeowner
                      </button>
                      <button 
                        className="btn btn-success"
                        onClick={() => {
                          const message = `✅ COMPLETED PROJECT\n\nProject: ${project.project_name}\nHomeowner: ${project.homeowner_name}\nEmail: ${project.homeowner_email}\nPhone: ${project.homeowner_phone || 'Not provided'}\nTotal Cost: ${project.technical_summary.total_cost}\nTimeline: ${project.timeline || 'TBD'}\nCompletion: ${project.project_summary.progress}\nCompleted On: ${project.project_summary.last_activity}`;
                          navigator.clipboard.writeText(message).then(() => {
                            try { toast.success('Project details copied to clipboard!'); } catch {}
                          }).catch(() => {
                            try { toast.error('Failed to copy details'); } catch {}
                      ...
                      <div className="details-grid">
                        ...
                        <div className="detail-section">
                          ...
                          <div className="detail-content">
                            ...
                            <p><strong>Labor Cost:</strong> {project.technical_summary.labor_cost}</p>
                            <p><strong>Timeline:</strong> {project.timeline || 'TBD'}</p>
                            <p><strong>Status:</strong> <span style={{color: '#10b981', fontWeight: 'bold'}}>Completed</span></p>
                            <p><strong>Created Date:</strong> {project.created_date_formatted}</p>
                            {project.materials && <p><strong>Materials:</strong> {project.materials}</p>}
                            {project.contractor_notes && <p><strong>Notes:</strong> {project.contractor_notes}</p>}
                          </div>
                        ...
                        <div className="detail-section" style={{
                          ...
                          <h4 className="section-title" style={{color: '#059669'}}>🎉 Completion Status</h4>
                          <div className="detail-content">
                            <p><strong>Final Progress:</strong> <span style={{color: '#10b981', fontWeight: 'bold'}}>{project.project_summary.progress}</span></p>
                            <p><strong>Final Stage:</strong> {project.project_summary.current_stage}</p>
                            <p><strong>Total Updates:</strong> {project.project_summary.updates_count}</p>
                            <p><strong>Completed On:</strong> {project.project_summary.last_activity}</p>
                            <p><strong>Project Duration:</strong> {project.timeline || 'As per schedule'}</p>
                          </div>
                        </div>

                        {/* Cost & Time Overrun Section */}
                        {projectOverruns[project.id] && (
                          <div className="detail-section" style={{
                            background: 'linear-gradient(135deg, #fef3c7, #fde68a)',
                            border: '2px solid #f59e0b'
                          }}>
                            <h4 className="section-title" style={{color: '#92400e'}}>📊 Cost & Time Performance</h4>
                            <div className="detail-content">
                              {/* Cost Overrun */}
                              <div style={{marginBottom: '16px', padding: '12px', background: 'white', borderRadius: '8px'}}>
                                <h5 style={{margin: '0 0 8px 0', color: '#374151', fontSize: '14px'}}>💰 Cost Analysis</h5>
                                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px'}}>
                                  <div>
                                    <span style={{color: '#6b7280'}}>Original Estimate:</span>
                                    <div style={{fontWeight: 'bold', color: '#111827'}}>
                                      ₹{projectOverruns[project.id].cost_overrun.original_estimate.toLocaleString('en-IN')}
                                    </div>
                                  </div>
                                  <div>
                                    <span style={{color: '#6b7280'}}>Total Cost:</span>
                                    <div style={{fontWeight: 'bold', color: '#111827'}}>
                                      ₹{projectOverruns[project.id].cost_overrun.total_project_cost.toLocaleString('en-IN')}
                                    </div>
                                  </div>
                                  <div style={{gridColumn: '1 / -1'}}>
                                    <span style={{color: '#6b7280'}}>Cost Difference:</span>
                                    <div style={{
                                      fontWeight: 'bold',
                                      fontSize: '16px',
                                      color: projectOverruns[project.id].cost_overrun.has_overrun ? '#dc2626' : 
                                             projectOverruns[project.id].cost_overrun.has_underrun ? '#059669' : '#6b7280'
                                    }}>
                                      {projectOverruns[project.id].cost_overrun.has_overrun ? '+' : ''}
                                      ₹{Math.abs(projectOverruns[project.id].cost_overrun.cost_difference).toLocaleString('en-IN')}
                                      <span style={{fontSize: '14px', marginLeft: '8px'}}>
                                        ({projectOverruns[project.id].cost_overrun.cost_overrun_percentage > 0 ? '+' : ''}
                                        {projectOverruns[project.id].cost_overrun.cost_overrun_percentage}%)
                                      </span>
                                      {projectOverruns[project.id].cost_overrun.has_overrun && 
                                        <span style={{marginLeft: '8px', fontSize: '12px'}}>🔴 Over Budget</span>
                                      }
                                      {projectOverruns[project.id].cost_overrun.has_underrun && 
                                        <span style={{marginLeft: '8px', fontSize: '12px'}}>🟢 Under Budget</span>
                                      }
                                      {!projectOverruns[project.id].cost_overrun.has_overrun && 
                                       !projectOverruns[project.id].cost_overrun.has_underrun && 
                                        <span style={{marginLeft: '8px', fontSize: '12px'}}>🟡 On Budget</span>
                                      }
                                    </div>
                                  </div>
                                </div>
                              ...
                              {/* Time Overrun */}
                              {projectOverruns[project.id].time_overrun.planned_duration_days > 0 && (
                                <div style={{padding: '12px', background: 'white', borderRadius: '8px'}}>
                                  <h5 style={{margin: '0 0 8px 0', color: '#374151', fontSize: '14px'}}>⏱️ Timeline Analysis</h5>
                                  <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px'}}>
                                    <div>
                                      <span style={{color: '#6b7280'}}>Planned Duration:</span>
                                      <div style={{fontWeight: 'bold', color: '#111827'}}>
                                        {projectOverruns[project.id].time_overrun.planned_duration_days} days
                                      </div>
                                    </div>
                                    <div>
                                      <span style={{color: '#6b7280'}}>Actual Duration:</span>
                                      <div style={{fontWeight: 'bold', color: '#111827'}}>
                                        {projectOverruns[project.id].time_overrun.actual_duration_days} days
                                      </div>
                                    </div>
                                    <div style={{gridColumn: '1 / -1'}}>
                                      <span style={{color: '#6b7280'}}>Time Difference:</span>
                                      <div style={{
                                        fontWeight: 'bold',
                                        fontSize: '16px',
                                        color: projectOverruns[project.id].time_overrun.has_overrun ? '#dc2626' : 
                                               projectOverruns[project.id].time_overrun.has_early_completion ? '#059669' : '#6b7280'
                                      }}>
                                        {projectOverruns[project.id].time_overrun.delay_days > 0 ? '+' : ''}
                                        {projectOverruns[project.id].time_overrun.delay_days} days
                                        <span style={{fontSize: '14px', marginLeft: '8px'}}>
                                          ({projectOverruns[project.id].time_overrun.time_overrun_percentage > 0 ? '+' : ''}
                                          {projectOverruns[project.id].time_overrun.time_overrun_percentage}%)
                                        </span>
                                        {projectOverruns[project.id].time_overrun.has_overrun && 
                                          <span style={{marginLeft: '8px', fontSize: '12px'}}>🔴 Delayed</span>
                                        }
                                        {projectOverruns[project.id].time_overrun.has_early_completion && 
                                          <span style={{marginLeft: '8px', fontSize: '12px'}}>🟢 Early</span>
                                        }
                                        {!projectOverruns[project.id].time_overrun.has_overrun && 
                                         !projectOverruns[project.id].time_overrun.has_early_completion && 
                                          <span style={{marginLeft: '8px', fontSize: '12px'}}>🟡 On Time</span>
                                        }
                                      </div>
                                    </div>
                                    <div style={{gridColumn: '1 / -1', marginTop: '8px', fontSize: '12px', color: '#6b7280'}}>
                                      <div>Planned: {projectOverruns[project.id].time_overrun.planned_start_date} → {projectOverruns[project.id].time_overrun.planned_end_date}</div>
                                      <div>Actual: {projectOverruns[project.id].time_overrun.actual_start_date} → {projectOverruns[project.id].time_overrun.actual_end_date}</div>
                                    </div>
                                  </div>
                                </div>
                              )}
                              ...
                              {/* Performance Rating */}
                              {projectOverruns[project.id].overall_performance && (
                                <div style={{marginTop: '12px', padding: '8px 12px', background: 'white', borderRadius: '8px', textAlign: 'center'}}>
                                  <span style={{fontSize: '12px', color: '#6b7280'}}>Overall Performance: </span>
                                  <span style={{
                                    fontWeight: 'bold',
                                    fontSize: '14px',
                                    color: projectOverruns[project.id].overall_performance.performance_rating === 'excellent' ? '#059669' :
                                           projectOverruns[project.id].overall_performance.performance_rating === 'good' ? '#10b981' :
                                           projectOverruns[project.id].overall_performance.performance_rating === 'fair' ? '#f59e0b' : '#dc2626'
                                  }}>
                                    {projectOverruns[project.id].overall_performance.performance_rating.toUpperCase()}
                                    {projectOverruns[project.id].overall_performance.both_on_target && ' 🎯'}
                                  </span>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="detail-section">
                          ...
                          <div className="detail-content">
                            <div style={{
                              ...
                              <h5 style={{margin: '0 0 12px 0', color: '#374151'}}>✅ Project Achievements</h5>
                              <ul style={{margin: 0, paddingLeft: '20px', lineHeight: '1.8'}}>
                                <li>✅ All construction stages completed</li>
                                <li>✅ {project.project_summary.updates_count} progress updates submitted</li>
                                <li>✅ Project delivered within timeline</li>
                                <li>✅ Quality standards maintained</li>
                                <li>✅ Client satisfaction achieved</li>
                              </ul>
                            </div>
                            <div style={{marginTop: '16px', padding: '12px', background: '#fef3c7', borderRadius: '8px', border: '1px solid #fbbf24'}}>
                              <p style={{margin: 0, fontSize: '14px', color: '#92400e'}}>
                                <strong>💡 Note:</strong> This project has been successfully completed. You can still view all project details and progress history.
                              </p>
                            </div>
                          </div>
                        </div>
      ...
      <div>
        ...
        {progressView === 'submit' ? (
          ...
          <div className="documents-section">
            <div className="documents-header" style={{ marginBottom: '24px' }}>
