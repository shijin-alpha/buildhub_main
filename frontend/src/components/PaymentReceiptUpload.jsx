...
const PaymentReceiptUpload = ({ 
  ...
  const uploadReceipt = async () => {
    ...
    try {
      ...
      // First, establish session to ensure authentication
      console.log('Establishing session...');
      try {
        // Try to establish session based on the payment ownership
        ...
        if (!sessionData.success) {
          console.warn('Session establishment failed:', sessionData.message);
        }
      } catch (sessionError) {
        console.warn('Session establishment failed, continuing anyway:', sessionError);
      }
      ...
      if (data.success) {
        ...
        if (errorMessage.includes('Payment not found') || errorMessage.includes('does not exist')) {
          errorMessage = 'This payment request is not available for receipt upload. The payment list may be outdated.';
          showRefreshOption = true;
        } else if (errorMessage.includes('different homeowner')) {
          errorMessage = 'This payment belongs to a different user. Please refresh the page to load your payments.';
          showRefreshOption = true;
        } else if (errorMessage.includes('not authenticated')) {
          errorMessage = 'Your session has expired. Please log in again.';
        } else if (errorMessage.includes('access denied')) {
          errorMessage = 'You do not have permission to upload receipts for this payment.';
        }
        
        toast.error('Failed to upload receipt: ' + errorMessage);
        
        // Show refresh suggestion if applicable
        if (showRefreshOption && data.suggestion) {
          setTimeout(() => {
        ...
        if (data.debug) {
          console.error('Detailed debug info:', {
            payment_id: data.debug.payment_id,
            homeowner_id: data.debug.homeowner_id,
            payment_exists: data.debug.payment_exists,
            payment_details: data.debug.payment_details,
  ...
  // Add validation for payment ID
  React.useEffect(() => {
    if (show && !paymentId) {
      console.error('PaymentReceiptUpload: No payment ID provided');
      toast.error('Invalid payment selected. Please try again.');
      onCancel();
    }
  }, [show, paymentId, onCancel, toast]);
