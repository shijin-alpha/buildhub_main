...
const HousePlanDrawer = ({ 
    ...
    try {
      ...
      if (assignment && assignment.layout_request) {
        ...
        setPlanData(prev => ({
          ...
          plot_height: parseFloat(request.plot_size) || 100,
          scale_ratio: prev.scale_ratio || 1.2
        }));

        // If an existing plan has rooms all on floor 1 but the request has multiple floors,
        // redistribute rooms across floors based on floor_rooms from the request
        const floorRoomsForRedist = requirements?.floor_rooms || request.floor_rooms || null;
        if (existingPlan && floorRoomsForRedist) {
          const floorKeys = Object.keys(floorRoomsForRedist).map(k => parseInt(k.replace(/\D/g, '')) || 1);
          const maxFloor = Math.max(...floorKeys);
          ...
          if (maxFloor > 1) {
            ...
            for (let f = 1; f <= maxFloor; f++) {
              names[f] = f === 1 ? 'Ground Floor' : `Floor ${f}`;
              offsets[f] = { x: 0, y: f === 1 ? 0 : (f - 1) * 300 };
            }
            setFloorNames(names);
            setFloorOffsets(offsets);

            // Only redistribute if all rooms are on floor 1 (legacy plan with no floor data)
            setPlanData(prev => {
              const allOnFloor1 = prev.rooms.every(r => Number(r.floor) === 1);
              if (!allOnFloor1) return prev;

              const floorAssignments = [];
              Object.entries(floorRoomsForRedist).forEach(([floorKey, floorRooms]) => {
                const floorNum = parseInt(floorKey.replace(/\D/g, '')) || 1;
                Object.entries(floorRooms).forEach(([roomType, count]) => {
                  const roomCount = typeof count === 'number' ? count : parseInt(count) || 1;
                  for (let i = 0; i < roomCount; i++) floorAssignments.push(floorNum);
                });
              });

              const redistributed = prev.rooms.map((room, idx) => {
                const targetFloor = floorAssignments[idx] != null ? floorAssignments[idx] : 1;
                return {
                  ...room,
                  floor: targetFloor,
                  y: targetFloor > 1 ? (room.y % 300) + (targetFloor - 1) * 300 : room.y
                };
              });
              return { ...prev, rooms: redistributed };
            });
          }
        }

        // Pre-populate rooms based on requirements
        // Only pre-populate if there are no existing rooms (don't overwrite a saved plan)
        if (requirements && !existingPlan && planData.rooms.length === 0) {
          ...
          // Use floor_rooms from requirements JSON, or fall back to the dedicated request.floor_rooms column
          const floorRoomsSource = requirements.floor_rooms || request.floor_rooms || null;

          // Handle floor_rooms if available
          if (floorRoomsSource) {
            Object.entries(floorRoomsSource).forEach(([floorKey, floorRooms]) => {
              // Handle keys like "floor1", "floor2", "1", "2"
              const floorNumber = parseInt(floorKey.replace(/\D/g, '')) || 1;
              let currentX = 50; // Starting X position for each floor (reset per floor)
              let floorY = 50;   // Starting Y position for each floor (reset per floor)
              // Floor offset: each floor is stacked 300px below the previous
              const floorOffsetY = (floorNumber - 1) * 300;
              ...
              Object.entries(floorRooms).forEach(([roomType, count]) => {
                ...
                for (let i = 0; i < roomCount; i++) {
                  ...
                  prePopulatedRooms.push({
                    ...
                    type: roomType,
                    x: currentX,
                    y: floorY + floorOffsetY, // Each floor starts at its own offset
                    layout_width: 10,
                    layout_height: 10,
                  ...
                  if (currentX > 400) { // Wrap to next row within this floor
                    currentX = 50;
                    floorY += 120;
                  }
                }
          ...
          if (prePopulatedRooms.length > 0) {
            ...
            // Sync floor state from floor_rooms keys
            if (floorRoomsSource) {
              const floorKeys = Object.keys(floorRoomsSource).map(k => parseInt(k.replace(/\D/g, '')) || 1);
              const maxFloor = Math.max(...floorKeys);
              setTotalFloors(maxFloor);
              const names = {};
              const offsets = {};
  ...
  useEffect(() => {
    if (existingPlan) {
      ...
      const newPlanData = {
        ...
        plot_width: existingPlan.plot_width || 100,
        plot_height: existingPlan.plot_height || 100,
        // Ensure every room has a floor property (fallback to 1 for legacy plans)
        rooms: (parsedPlanData?.rooms || []).map(room => ({
          ...room,
          floor: room.floor != null ? Number(room.floor) : 1
        })),
        notes: existingPlan.notes || '',
        scale_ratio: parsedPlanData?.scale_ratio || 1.2
      };
      
      // Determine floor_rooms from requestInfo prop (for redistribution)
      let floorRoomsFromRequest = null;
      if (requestInfo?.layout_request) {
        const req = requestInfo.layout_request;
        let reqs = null;
        try {
          reqs = typeof req.requirements === 'string' ? JSON.parse(req.requirements) : req.requirements;
        } catch (e) {}
        floorRoomsFromRequest = reqs?.floor_rooms || req.floor_rooms || null;
      }

      // Determine totalFloors: prefer request floor_rooms, then saved floors metadata, then derive from rooms
      let resolvedMaxFloor = 1;
      if (floorRoomsFromRequest) {
        const fkeys = Object.keys(floorRoomsFromRequest).map(k => parseInt(k.replace(/\D/g, '')) || 1);
        resolvedMaxFloor = Math.max(...fkeys);
      } else if (parsedPlanData?.floors?.total_floors) {
        resolvedMaxFloor = parsedPlanData.floors.total_floors;
      } else if (newPlanData.rooms.length > 0) {
        resolvedMaxFloor = Math.max(...newPlanData.rooms.map(r => Number(r.floor) || 1));
      }

      // Build floor names/offsets
      const resolvedNames = parsedPlanData?.floors?.floor_names || {};
      const resolvedOffsets = parsedPlanData?.floors?.floor_offsets || {};
      for (let f = 1; f <= resolvedMaxFloor; f++) {
        if (!resolvedNames[f]) resolvedNames[f] = f === 1 ? 'Ground Floor' : `Floor ${f}`;
        if (!resolvedOffsets[f]) resolvedOffsets[f] = { x: 0, y: f === 1 ? 0 : (f - 1) * 300 };
      }

      setTotalFloors(resolvedMaxFloor);
      setCurrentFloor(1);
      setFloorNames(resolvedNames);
      setFloorOffsets(resolvedOffsets);

      // If all rooms are on floor 1 but request has multiple floors, redistribute
      if (resolvedMaxFloor > 1 && floorRoomsFromRequest) {
        const allOnFloor1 = newPlanData.rooms.every(r => Number(r.floor) === 1);
        if (allOnFloor1 && newPlanData.rooms.length > 0) {
          const floorAssignments = [];
          Object.entries(floorRoomsFromRequest).forEach(([floorKey, floorRooms]) => {
            const floorNum = parseInt(floorKey.replace(/\D/g, '')) || 1;
            Object.entries(floorRooms).forEach(([roomType, count]) => {
              const roomCount = typeof count === 'number' ? count : parseInt(count) || 1;
              for (let i = 0; i < roomCount; i++) floorAssignments.push(floorNum);
          ...
          newPlanData.rooms = newPlanData.rooms.map((room, idx) => {
            const targetFloor = floorAssignments[idx] != null ? floorAssignments[idx] : 1;
            return {
              ...room,
              floor: targetFloor,
      ...
      console.log('Setting plan data:', newPlanData, 'totalFloors:', resolvedMaxFloor); // Debug log
  ...
  }, [existingPlan?.id, requestInfo]); // Re-run when plan or request info changes
  ...
  const drawMeasurements = () => {
    ...
    const currentFloorRoomsForMeasure = planData.rooms.filter(room => Number(room.floor) === Number(currentFloor));
    currentFloorRoomsForMeasure.forEach(room => {
      const x = room.x + 20;
      const y = room.y + 20;
      const width = room.layout_width * PIXELS_PER_FOOT;
      const height = room.layout_height * PIXELS_PER_FOOT;
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Draw ghost overlay of floor 1 rooms when on upper floors (for reference)
    if (currentFloor > 1) {
      const groundFloorRooms = planData.rooms.filter(room => Number(room.floor) === 1);
      canvas.save();
      ...
      groundFloorRooms.forEach((room) => {
        ...
        const width = room.layout_width * PIXELS_PER_FOOT;
        const height = room.layout_height * PIXELS_PER_FOOT;
        canvas.fillStyle = room.color || '#90caf9';
        canvas.fillRect(x, y, width, height);
        ...
        canvas.font = '9px Arial';
        canvas.textAlign = 'center';
        canvas.fillText(room.name, x + width / 2, y + height / 2);
      });
  ...
  }, [canvas, planData, selectedRoom, showMeasurements, measurementMode, highlightedRoomType, isRotating, currentFloor]);

  // Canvas setup: only resize when plot dimensions change or on first mount
  useEffect(() => {
  ...
  }, [planData.plot_width, planData.plot_height]);

  // Drawing: re-draw whenever any visual state changes (floor switch, rooms, selection, etc.)
  useEffect(() => {
    drawCanvas();
  }, [drawCanvas]);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (canvasRef.current) {
        ...
        // Update context after resize then redraw
  ...
  const removeFloor = (floorNumber) => {
    ...
    setPlanData(prev => ({
      ...prev,
      rooms: prev.rooms.filter(room => Number(room.floor) !== Number(floorNumber))
    }));
  ...
  const updateFloorOffset = (floorNumber, offsetX, offsetY) => {
    ...
    setPlanData(prev => ({
      ...
      rooms: prev.rooms.map(room => {
        if (Number(room.floor) === Number(floorNumber)) {
  ...
  const getCurrentFloorRooms = () => {
    return planData.rooms.filter(room => Number(room.floor) === Number(currentFloor));
  };
    ...
    try {
      ...
      if (planData.rooms && planData.rooms.length > 0) {
        ...
        for (let floorNum = 1; floorNum <= totalFloors; floorNum++) {
          const floorRooms = planData.rooms.filter(room => Number(room.floor) === floorNum);
  ...
  const handleTechnicalDetailsSubmit = async (technicalDetails) => {
    ...
    try {
      ...
      // Generate layout image from canvas
      ...
      // Submit with technical details and layout image
      const submissionPayload = {
        plan_id: currentPlanId,
        technical_details: technicalDetails,
        layout_image_data: layoutImageData, // NEW: Include layout image
        plan_data: {
          rooms: planData.rooms,
          scale_ratio: planData.scale_ratio,
          total_layout_area: calculateTotalArea(),
      ...
      console.log('Submitting with technical details and layout image:', {
        ...submissionPayload,
        layout_image_data: layoutImageData ? 'Image data captured' : 'No image data'
      });
      ...
      if (result.success) {
        ...
        showSuccess(
          ...
          `Your house plan "${planData.plan_name}" with technical details and layout design has been submitted to the homeowner for review.`
        );
